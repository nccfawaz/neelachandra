...
**Full production secret list (§29.70, cut-over preflight).** What each
is, what generates it, what breaks if it is lost, and when it must exist:

| Secret | Generated by | If lost | Before first 2FA enrolment? |
|---|---|---|---|
| `SESSION_SECRET` | operator (32+ random bytes, `openssl rand -hex 32`) | all live sessions invalidated on rotation; recoverable by rotating | no |
| `TOTP_ENCRYPTION_KEY` | operator (32+ random bytes) | every enrolled TOTP secret unreadable — enrolled accounts locked out, recovery codes the only way back; **cannot be regenerated** | **YES — must be final** |
| `DB_PASSWORD` | hPanel database creation | app fails boot (env validation, named error) | no |
| staff / owner passwords | `seed-staff.mjs --reset-passwords` (argon2-hashed at rest) | re-seed + reset; no data loss | no |
| dormant-admin recovery codes | seed script, printed once | if lost with the phone, that admin is unreachable — treat as lost key | at enrolment, held offline |

`scripts/preflight.mjs` checks the first three at cut-over; the recovery
codes are a physical-custody item and no script can verify them.
