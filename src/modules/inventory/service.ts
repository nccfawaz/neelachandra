...
/**
 * The users who should hear about something, resolved by permission key
 ...
 * matching on a role key would break the day a role is renamed. A permission
 * key is the durable identity of a duty here.
 *
 ...
 */
async function usersWithPermission(db: Queryable, key: string): Promise<number[]> {
...
async function notifyPermission(
  db: Queryable,
  key: string,
  opts: { actorId: number; kind: string; title: string; body?: string | null; linkPath?: string | null; severity?: 'info' | 'warn' | 'critical' }
): Promise<void> {
