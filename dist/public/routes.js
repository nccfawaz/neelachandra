...
async function serve(c, file, cacheControl) {
 ...
 */
async function handleEnquiry(c) {
    const db = c.get('db');
    const body = await readBody(c);
    const sourcePage = new URL(c.req.url).pathname;
    const notifyTo = await getSetting(db, 'company.email_enquiry', '');
    ...
    if (c.req.header('hx-request')) {
        return outcome.ok
            ? c.html('<div class="ncc-alert ncc-alert--ok" role="status"><strong>Thank you.</strong> Your enquiry has reached us and we will be in touch shortly.</div>')
            : c.html(`<div class="ncc-alert ncc-alert--error" role="alert">${escapeHtml(outcome.message ?? 'Please check the form.')}</div>`, 422);
    }
