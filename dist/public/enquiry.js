 ...
 */
function serviceInterest(input) {
    const parts = [];
...
export async function submitEnquiry(db, raw, meta) {
    // Rate limit before validation. Validating first would let an attacker
    // spend our CPU on Zod for free by sending garbage.
    await enforce(db, RULES.enquiryByIp(meta.ip ?? 'unknown'), 'Too many enquiries from this connection. Please try again later, or call us directly.');
