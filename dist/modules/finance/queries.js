 ...
 */
export async function costHeadStates(trx, projectId) {
  ...
  `.execute(trx);
    ...
    const heads = new Set([...budgetByHead.keys(), ...committedByHead.keys(), ...actualByHead.keys()]);
 ...
 */
export async function periodForDate(trx, date) {
    const row = await trx
