...
function idParam(c, name = 'expenseId') {
...
finance.get('/app/finance/budgets', requirePermission(PERMISSIONS.FINANCE_VIEW_PROJECT_BUDGET), async (c) => {
    ...
    return page(c, { title: 'Budgets', path: '/app/finance/budgets' }, _jsxs(_Fragment, { children: [banner(c), _jsx("div", { class: "ncc-kpi-row", children: _jsx(KpiCard, { label: "Records held", value: String(total), hint: "Live count from project_budgets" }) }), _jsx(Panel, { title: "Budgets", children: _jsx(Alert, { tone: "warn", children: "The data model behind this screen is migrated. The entry and approval forms are the next build phase." }) })] }));
});
finance.get('/app/finance/expenses', requirePermission(PERMISSIONS.FINANCE_EXPENSE_CREATE), async (c) => {
    ...
    const row = await db
        ...
        .executeTakeFirst();
    const total = Number(row?.n ?? 0);
    return page(c, { title: 'Expenses', path: '/app/finance/expenses' }, _jsxs(_Fragment, { children: [banner(c), _jsx("div", { class: "ncc-kpi-row", children: _jsx(KpiCard, { label: "Records held", value: String(total), hint: "Live count from expenses" }) }), _jsx(Panel, { title: "Expenses", children: _jsx(Alert, { tone: "warn", children: "The data model behind this screen is migrated. The entry and approval forms are the next build phase." }) })] }));
});
finance.get('/app/finance/invoices', requirePermission(PERMISSIONS.FINANCE_INVOICE_MANAGE), async (c) => {
    ...
    const row = await db
        ...
        .executeTakeFirst();
    const total = Number(row?.n ?? 0);
    return page(c, { title: 'Client invoices', path: '/app/finance/invoices' }, _jsxs(_Fragment, { children: [banner(c), _jsx("div", { class: "ncc-kpi-row", children: _jsx(KpiCard, { label: "Records held", value: String(total), hint: "Live count from client_invoices" }) }), _jsx(Panel, { title: "Client invoices", children: _jsx(Alert, { tone: "warn", children: "The data model behind this screen is migrated. The entry and approval forms are the next build phase." }) })] }));
});
finance.get('/app/finance/payments', requirePermission(PERMISSIONS.FINANCE_PAYMENT_RECORD), async (c) => {
    ...
    const row = await db
        ...
        .executeTakeFirst();
    const total = Number(row?.n ?? 0);
    return page(c, { title: 'Payments', path: '/app/finance/payments' }, _jsxs(_Fragment, { children: [banner(c), _jsx("div", { class: "ncc-kpi-row", children: _jsx(KpiCard, { label: "Records held", value: String(total), hint: "Live count from payments" }) }), _jsx(Panel, { title: "Payments", children: _jsx(Alert, { tone: "warn", children: "The data model behind this screen is migrated. The entry and approval forms are the next build phase." }) })] }));
});
finance.get('/app/finance/periods', requirePermission(PERMISSIONS.FINANCE_PERIOD_CLOSE), async (c) => {
    ...
    const row = await db
        ...
        .executeTakeFirst();
    const total = Number(row?.n ?? 0);
    return page(c, { title: 'Accounting periods', path: '/app/finance/periods' }, _jsxs(_Fragment, { children: [banner(c), _jsx("div", { class: "ncc-kpi-row", children: _jsx(KpiCard, { label: "Records held", value: String(total), hint: "Live count from accounting_periods" }) }), _jsx(Panel, { title: "Accounting periods", children: _jsx(Alert, { tone: "warn", children: "The data model behind this screen is migrated. The entry and approval forms are the next build phase." }) })] }));
});
...
finance.post('/app/finance/expenses', requirePermission(PERMISSIONS.FINANCE_EXPENSE_CREATE), async (c) => {
    ...
    if (!parsed.success)
        return errRedirect(c, '/app/finance/expenses', firstError(parsed.error));
...
finance.post('/app/finance/payments', requirePermission(PERMISSIONS.FINANCE_PAYMENT_RECORD), async (c) => {
    ...
    if (!parsed.success)
        return errRedirect(c, '/app/finance/payments', firstError(parsed.error));
...
finance.post('/app/finance/advances', requirePermission(PERMISSIONS.FINANCE_PAYMENT_RECORD), async (c) => {
    ...
    if (!parsed.success)
        return errRedirect(c, '/app/finance/advances', firstError(parsed.error));
...
finance.post('/app/finance/invoices', requirePermission(PERMISSIONS.FINANCE_INVOICE_MANAGE), async (c) => {
    ...
    if (!parsed.success)
        return errRedirect(c, '/app/finance/invoices', firstError(parsed.error));
