 ...
 */
export async function createExpense(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        ...
        await trx.insertInto('expense_lines').values(input.lines.map((l) => ({
        ...
        return { expenseId, expenseNo, status: 'draft' };
    });
}
/**
 ...
 */
export async function submitExpense(db, actor, expenseId) {
    return db.transaction().execute(async (trx) => {
        const expense = await trx
            ...
            .executeTakeFirst();
        if (!expense)
            throw new NotFoundError('That expense does not exist.');
 ...
 */
export async function approveExpense(db, actor, expenseId, roleKeys) {
    return db.transaction().execute(async (trx) => {
        const expense = await trx
            .selectFrom('expenses')
            .select([
        ...
        if (expense.status !== 'pending_approval') {
            throw new ConflictError(`This expense is ${String(expense.status).replace(/_/g, ' ')}. Only one awaiting approval can be approved.`);
        }
        if (Number(expense.created_by) === actor.userId) {
            throw new ForbiddenError('You entered this expense, so you cannot approve it. Someone else holding the approval permission has to.');
        }
        ...
        if (expense.project_id !== null) {
            ...
            for (const line of lines) {
                ...
                if (!state || state.budgetPaise === null)
                    continue;
                ...
                if (projected > state.budgetPaise) {
                    ...
                    throw new UnprocessableError(`Approving this expense would push cost head ${line.cost_head_id} past its budget: ` +
        ...
        if (needsSecond && isFirst) {
            ...
            return { expenseNo: expense.expense_no, status: 'awaiting_second_approval', totalPaise: total };
        }
        ...
        return { expenseNo: expense.expense_no, status: 'approved', totalPaise: total };
    });
}
/**
 ...
 */
export async function createPayment(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        if (input.bankAccountId !== null) {
            ...
            if (!bank)
                throw new UnprocessableError('That bank account no longer exists.');
        }
        const inserted = await trx
            .insertInto('payments')
            .values({
 ...
 */
async function allocatePaymentRows(trx, actor, paymentId, allocations) {
    for (const alloc of allocations) {
        if (alloc.documentType === 'expense') {
            const doc = await trx
                ...
                .executeTakeFirst();
            if (!doc)
                throw new NotFoundError(`Expense ${alloc.documentId} does not exist.`);
            if (doc.status === 'void')
                throw new UnprocessableError(`Expense ${alloc.documentId} is void and cannot be paid.`);
            const existing = Number(doc.paid_paise ?? 0);
            const projected = existing + alloc.allocatedPaise;
            if (projected > Number(doc.total_paise)) {
                throw new UnprocessableError(`Allocating ${formatPaiseAsRupees(alloc.allocatedPaise)} would take expense ${alloc.documentId} to ` +
            ...
            const doc = await trx
                ...
                .executeTakeFirst();
            if (!doc)
                throw new NotFoundError(`Contractor bill ${alloc.documentId} does not exist.`);
            ...
            if (projected > Number(doc.net_payable_paise)) {
                throw new UnprocessableError(`Allocating ${formatPaiseAsRupees(alloc.allocatedPaise)} would take contractor bill ${alloc.documentId} to ` +
            ...
            const doc = await trx
                ...
                .executeTakeFirst();
            if (!doc)
                throw new NotFoundError(`Client invoice ${alloc.documentId} does not exist.`);
            ...
            if (projected > Number(doc.net_receivable_paise)) {
                throw new UnprocessableError(`Allocating ${formatPaiseAsRupees(alloc.allocatedPaise)} would take client invoice ${alloc.documentId} to ` +
            ...
            throw new UnprocessableError(`Allocations against advance documents land with the slice that builds them. Expenses, contractor bills and client invoices are the payable documents today.`);
        }
    }
    void actor;
}
 ...
 * than it paid.
 */
export async function allocatePayment(db, actor, paymentId, input) {
    return db.transaction().execute(async (trx) => {
        ...
        if (projected > Number(payment.amount_paise)) {
            throw new UnprocessableError(`These allocations would take payment ${payment.payment_no} to ${formatPaiseAsRupees(projected)} ` +
 ...
 */
export async function msmeAgeing(db, onDate) {
    const rows = await db
        .selectFrom('expenses')
 ...
 */
export async function openAdvanceOutstanding(db, employeeId) {
    const rows = await db
        .selectFrom('expenses')
 ...
 */
export async function issueSiteAdvance(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        ...
        if (outstanding + input.amountPaise > Number(threshold)) {
            throw new UnprocessableError(`This employee already has ${formatPaiseAsRupees(outstanding)} of open site advances. ` +
