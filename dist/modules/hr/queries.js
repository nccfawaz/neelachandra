...
export async function listEmployees(db, opts = {}) {
    let query = db
    ...
    if (opts.status)
        query = query.where('employees.status', '=', opts.status);
    if (opts.departmentId)
        query = query.where('employees.department_id', '=', opts.departmentId);
    if (opts.q)
        query = query.where('employees.full_name', 'like', `%${opts.q}%`);
    return (await query.execute());
}
export async function findEmployee(db, id) {
    return db
        .selectFrom('employees')
        .leftJoin('departments', 'departments.id', 'employees.department_id')
        .leftJoin('designations', 'designations.id', 'employees.designation_id')
 ...
 */
export async function compensationHistory(db, employeeId) {
    return db
...
export async function employeeDocuments(db, employeeId) {
    return db
...
export async function designationOptions(db) {
    return db
...
export async function locationOptions(db) {
...
export async function managerOptions(db, excludeId) {
    let query = db
        .selectFrom('employees')
 ...
 * in DECISIONS.md.
 */
export async function employeeLoginId(db, employeeId, userId) {
    if (userId)
 ...
 */
export async function exitBlockers(db, employeeId) {
    ...
    return {
        ...
        equipment: equipment,
        expensesRaised: expensesRaised,
        advancesOutstanding: advancesOutstanding,
    };
}
export function blockerCount(b) {
...
export async function attendanceRoster(db, month, opts = {}) {
    ...
    let query = db
        ...
        .where((eb) => eb.or([eb('employees.date_of_exit', 'is', null), eb('employees.date_of_exit', '>=', start)]))
        .orderBy('employees.employee_code');
    if (opts.employeeId)
...
export async function attendanceMonth(db, month, opts = {}) {
    ...
    if (opts.employeeId)
        query = query.where('attendance.employee_id', '=', opts.employeeId);
    if (opts.projectId)
        query = query.where('attendance.project_id', '=', opts.projectId);
    return (await query.execute());
}
/**
 ...
 */
export async function attendanceOn(db, date) {
    return (await db
        .selectFrom('attendance')
        .leftJoin('projects', 'projects.id', 'attendance.project_id')
        .select([
...
export async function attendanceMonthState(db, month) {
    const { start, end } = monthBounds(month);
    const row = await db
        .selectFrom('attendance')
        .select((eb) => [
        eb.fn.countAll().as('total'),
        eb.fn.count('attendance.approved_at').as('approved'),
    ])
        .where('attendance.attendance_date', '>=', start)
        .where('attendance.attendance_date', '<=', end)
        .executeTakeFirst();
...
export async function projectOptions(db) {
    return db
        .selectFrom('projects')
        .select(['id', 'code', 'name'])
...
/* Leave ------------------------------------------------------------------- */
export async function leaveTypeOptions(db) {
    return db
...
export async function approvedLeaveOn(db, date) {
    const rows = await db
...
export async function approvedLeaveMonth(db, month) {
    const { start, end } = monthBounds(month);
    const rows = await db
        .selectFrom('leave_requests')
        .innerJoin('leave_types', 'leave_types.id', 'leave_requests.leave_type_id')
 ...
 */
export async function listLeaveRequests(db, opts = {}) {
    let query = db
        .selectFrom('leave_requests')
    ...
    if (opts.employeeId)
        query = query.where('leave_requests.employee_id', '=', opts.employeeId);
    if (opts.status)
        query = query.where('leave_requests.status', '=', opts.status);
    ...
    if (opts.pendingFor)
        query = query.where('leave_requests.employee_id', '!=', opts.pendingFor);
    return (await query.execute());
}
export async function findLeaveRequest(db, id) {
    return db
        .selectFrom('leave_requests')
        .innerJoin('leave_types', 'leave_types.id', 'leave_requests.leave_type_id')
        .innerJoin('employees', 'employees.id', 'leave_requests.employee_id')
 ...
 */
export async function leaveBalances(db, employeeId, financialYear) {
    const rows = await db
        .selectFrom('leave_types')
        .leftJoin('leave_balances', (join) => join
...
export async function hrDashboard(db) {
...
export async function listContractors(db, opts = {}) {
    let query = db
    ...
    if (opts.status)
        query = query.where('labour_contractors.status', '=', opts.status);
    if (opts.search) {
        const like = `%${opts.search}%`;
        query = query.where((eb) => eb.or([eb('labour_contractors.name', 'like', like), eb('labour_contractors.code', 'like', like)]));
    }
    return (await query.execute());
}
export async function findContractor(db, id) {
    return db
        .selectFrom('labour_contractors')
        .leftJoin('vendors', 'vendors.id', 'labour_contractors.vendor_id')
        .select([
...
export async function contractorOptions(db) {
    return db
        .selectFrom('labour_contractors')
 ...
 */
export async function contractorVendorOptions(db) {
    return db
        .selectFrom('vendors')
        .select(['id', 'code', 'name'])
        .where('status', '=', 'active')
...
export async function contractorRates(db, contractorId) {
    const rows = await db
        ...
        .execute();
    return rows.map((r) => ({ ...r, rate_paise: Number(r.rate_paise) }));
}
/**
 ...
 */
export async function applicableRate(db, opts) {
    ...
    let query = db
        ...
        .where((eb) => eb.or([eb('skill_level', 'is', null), eb('skill_level', '=', opts.skillLevel)]));
    ...
    const ranked = [...rows].sort((a, b) => {
        const scope = (r) => (r.project_id === null ? 0 : 1);
        const skill = (r) => (r.skill_level === null ? 0 : 1);
        if (scope(a) !== scope(b))
            return scope(b) - scope(a);
        if (skill(a) !== skill(b))
            return skill(b) - skill(a);
        if (a.effective_from !== b.effective_from)
            return String(a.effective_from) < String(b.effective_from) ? 1 : -1;
 ...
 */
export async function contractorAttendance(db, opts) {
    let query = db
    ...
    if (opts.contractorId)
        query = query.where('contractor_attendance.contractor_id', '=', opts.contractorId);
    if (opts.projectId)
        query = query.where('contractor_attendance.project_id', '=', opts.projectId);
    if (opts.from)
        query = query.where('contractor_attendance.attendance_date', '>=', opts.from);
    if (opts.to)
        query = query.where('contractor_attendance.attendance_date', '<=', opts.to);
    if (opts.billId)
        query = query.where('contractor_attendance.bill_id', '=', opts.billId);
    if (opts.unbilledOnly)
        query = query.where('contractor_attendance.bill_id', 'is', null);
    if (opts.approvedOnly)
        query = query.where('contractor_attendance.approved_at', 'is not', null);
    const rows = await query.execute();
    return rows.map((r) => ({
        ...r,
 ...
 */
export async function unbilledSummary(db, opts) {
...
export async function listContractorBills(db, opts = {}) {
    let query = db
    ...
    if (opts.contractorId)
        query = query.where('contractor_bills.contractor_id', '=', opts.contractorId);
    if (opts.projectId)
        query = query.where('contractor_bills.project_id', '=', opts.projectId);
    if (opts.status)
        query = query.where('contractor_bills.status', '=', opts.status);
    return (await query.execute()).map(billRow);
}
/**
 ...
 */
export async function findContractorBill(db, id) {
    const row = await db
        .selectFrom('contractor_bills')
        .innerJoin('labour_contractors', 'labour_contractors.id', 'contractor_bills.contractor_id')
        .innerJoin('projects', 'projects.id', 'contractor_bills.project_id')
...
export async function checkinSiteOptions(db) {
    ...
    const office = await db
        ...
        .limit(1)
        .execute();
    const out = [{ key: 'site', label: 'Site', lat: null, lng: null }];
...
export async function resolveCheckinSite(db, key) {
    if (key === 'site')
        return { name: 'Site', lat: null, lng: null };
...
export async function farChecksOn(db, date) {
    ...
    for (const r of rows) {
        ...
        const flagOf = (at, lat, lng, far, siteKey) => {
            if (at === null)
                return '';
