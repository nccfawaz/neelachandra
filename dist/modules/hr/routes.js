...
hr.get('/app/hr/employees', requirePermission(PERMISSIONS.HR_EMPLOYEE_VIEW), async (c) => {
    ...
    return page(c, {
        ...
        actions: canManage(c) ? (_jsx("a", { class: "ncc-btn ncc-btn-primary", href: "/app/hr/employees/new", children: "New employee" })) : null,
                        ...
                        ] }), _jsx(FormField, { label: "Name contains", name: "q", value: search ?? '' }), _jsx("button", { class: "ncc-btn", type: "submit", children: "Filter" })] }), _jsx(Panel, { title: "Employees", children: _jsx(DataTable, { columns: columns, rows: rows, empty: "No employee matches that filter.", card: {
                        href: (r) => `/app/hr/employees/${r.id}`,
                        primary: (r) => (_jsxs(_Fragment, { children: [_jsx("strong", { children: r.full_name }), _jsx("span", { class: "ncc-listcard__code", children: r.employee_code })] })),
                        secondary: (r) => `${r.designation_name ?? '—'} · ${r.department_name ?? '—'}`,
                        status: (r) => _jsx(StatusBadge, { status: r.status }),
                    } }) })] }));
});
...
export function AttendanceGrid(props) {
    ...
    const table = (_jsx(DataTable, { columns: columns, rows: props.roster, empty: "Nobody was on the books in this month.", caption: props.filtered
            ...
            : undefined }));
    if (!editable) {
        return (_jsxs(_Fragment, { children: [_jsx("div", { class: "ncc-matrix", children: table }), _jsx(MarkLegend, { keys: false })] }));
    }
    ...
    return page(c, {
        ...
        actions: (_jsxs(_Fragment, { children: [_jsx("a", { class: "ncc-btn", href: "/app/hr/contractor-attendance", children: "Attendance entry" }), _jsx("a", { class: "ncc-btn", href: "/app/hr/contractor-bills", children: "Bills" }), _jsx("a", { class: "ncc-btn ncc-btn-primary", href: "/app/hr/contractors/new", children: "New contractor" })] })),
                            ...
                            status: (r) => {
                                ...
                                return lic || wc ? (_jsx("span", { class: "ncc-badge ncc-badge-danger", children: lic && wc ? 'licence & WC expired' : lic ? 'licence expired' : 'WC expired' })) : (_jsx(StatusBadge, { status: r.status }));
                            },
                        } }), _jsx("p", { class: "ncc-hint", children: "These are firms, not employees. Nothing on this screen creates a row in the employee master, and a contractor's workers are counted by skill level rather than named, because their employment sits with the firm, not with you." })] })] }));
});
/**
...
hr.get('/app/hr/attendance/far', requirePermission(PERMISSIONS.HR_ATTENDANCE_RECORD), async (c) => {
    ...
    const mapsLink = (r) => {
        if (r.lat === null || r.lng === null)
            return _jsx("span", { class: "ncc-muted", children: "no reading" });
        return (_jsxs("a", { href: `https://www.google.com/maps?q=${r.lat},${r.lng}`, target: "_blank", rel: "noopener noreferrer", children: [Number(r.lat).toFixed(6), ", ", Number(r.lng).toFixed(6), " \u2197"] }));
    };
    return page(c, {
