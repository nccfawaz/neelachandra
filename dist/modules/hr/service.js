 ...
 */
function auditableEmployee(input) {
    return {
 ...
 */
async function assertNoReportingCycle(trx, employeeId, managerId) {
    if (managerId === null)
        return;
    if (managerId === employeeId)
        throw new UnprocessableError('An employee cannot report to themselves.');
 ...
 */
export async function createEmployee(db, actor, input) {
...
export async function updateEmployee(db, actor, employeeId, input) {
 ...
 */
export async function reviseCompensation(db, actor, employeeId, input) {
    return db.transaction().execute(async (trx) => {
        ...
        if (open && String(open.effective_from) >= input.effectiveFrom) {
            throw new ConflictError(`A compensation period already starts on ${String(open.effective_from)}. A revision must take effect after it.`);
        }
        if (open) {
            await trx
...
export async function addEmployeeDocument(db, actor, employeeId, input) {
    return db.transaction().execute(async (trx) => {
        const employee = await trx
            .selectFrom('employees')
            .select(['id'])
        ...
        if (!file)
            throw new UnprocessableError('That attachment no longer exists.');
        const inserted = await trx
 ...
 */
export async function runExit(db, actor, employeeId, input) {
    return db.transaction().execute(async (trx) => {
        const employee = await trx
            .selectFrom('employees')
        ...
        if (!employee)
            throw new NotFoundError('Employee not found');
        if (employee.status === 'exited')
            throw new ConflictError('That employee has already exited.');
        ...
        if (outstanding > 0 && !input.override) {
            ...
            throw new UnprocessableError(`${subject} with this employee. Clear ${outstanding === 1 ? 'it' : 'them'}, or record a reason to complete the exit anyway.`);
        }
        await trx
            .updateTable('employees')
            .set({
 ...
 */
async function assertMonthOpen(trx, month, canOverride) {
    if (canOverride)
        return;
    const state = await attendanceMonthState(trx, month);
    if (state.locked) {
        throw new UnprocessableError(`${formatMonth(month)} is closed: ${state.approved} of its ${state.total} attendance rows are approved. Reopening a closed month needs finance.period_close, because a payroll figure that changes after the payment is made is the thing this lock prevents.`);
    }
}
/**
 ...
 */
export async function recordAttendanceBulk(db, actor, input, opts) {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        for (const row of input.rows) {
            ...
            if (employee.date_of_exit !== null && input.attendanceDate > String(employee.date_of_exit)) {
                throw new UnprocessableError(`${employee.full_name} left on ${String(employee.date_of_exit)} and cannot be marked after that.`);
            }
            const leave = leaveByEmployee.get(row.employeeId);
            if (leave && !LEAVE_DAY_STATUSES.includes(row.status)) {
                throw new UnprocessableError(`${employee.full_name} has approved leave covering ${input.attendanceDate} (request ${leave.id}). Withdraw the request before marking that day differently, so the leave balance and the attendance do not disagree.`);
            }
            const prior = priorByEmployee.get(row.employeeId);
            if (prior && prior.approved_at !== null && !opts.canOverridePeriod) {
                throw new UnprocessableError(`${employee.full_name}'s attendance for ${input.attendanceDate} is already approved. Changing it needs finance.period_close.`);
            }
            if (prior) {
                await trx
 ...
 * safe to repeat.
 */
export async function recordAttendanceGrid(db, actor, input, opts) {
    return db.transaction().execute(async (trx) => {
        ...
        const priorRows = await trx
            ...
            .where('employee_id', 'in', ids)
            .execute();
        const priorByCell = new Map(priorRows.map((r) => [`${Number(r.employee_id)}|${String(r.attendance_date)}`, r]));
        ...
        for (const cell of input.cells) {
            ...
            if (cell.date > now) {
                throw new UnprocessableError(`${cell.date} has not happened yet. Attendance is recorded for a day that has passed.`);
            }
            if (cell.date < String(employee.date_of_joining)) {
                throw new UnprocessableError(`${who} joined on ${String(employee.date_of_joining)} and cannot be marked on ${cell.date}.`);
            }
            if (employee.date_of_exit !== null && cell.date > String(employee.date_of_exit)) {
                throw new UnprocessableError(`${who} left on ${String(employee.date_of_exit)} and cannot be marked on ${cell.date}.`);
            }
            const leave = leaveByCell.get(key);
            if (leave && !LEAVE_DAY_STATUSES.includes(cell.status)) {
                throw new UnprocessableError(`${who} has approved leave covering ${cell.date} (request ${leave.request_id}). Withdraw the request before marking that day differently, so the leave balance and the attendance do not disagree.`);
            }
            if (prior && prior.approved_at !== null && !opts.canOverridePeriod) {
                throw new UnprocessableError(`${who}'s attendance for ${cell.date} is already approved. Changing it needs finance.period_close.`);
            }
            if (prior) {
                await trx
                    .updateTable('attendance')
 ...
 */
export async function approveAttendanceMonth(db, actor, month) {
    return db.transaction().execute(async (trx) => {
        const state = await attendanceMonthState(trx, month);
        if (state.total === 0) {
 ...
 */
export async function requestLeave(db, actor, input, opts) {
    ...
    if (employeeId === null) {
        throw new UnprocessableError('Your login is not linked to an employee record, so there is no one to book this leave against. Ask HR to link them.');
    }
    return db.transaction().execute(async (trx) => {
        const employee = await trx
            .selectFrom('employees')
        ...
        if (input.fromDate < String(employee.date_of_joining)) {
            throw new UnprocessableError(`Leave cannot start before the date of joining, ${String(employee.date_of_joining)}.`);
        }
        const type = await trx
            ...
            .executeTakeFirst();
        if (!type)
            throw new UnprocessableError('That leave type no longer exists.');
        if (Number(type.is_active) !== 1)
            throw new UnprocessableError(`${type.name} is no longer in use.`);
        const days = input.halfDay ? 0.5 : workingDaysBetween(input.fromDate, input.toDate);
        if (days <= 0) {
            throw new UnprocessableError('That range contains no working days. A Sunday is already the weekly off, so there is nothing to book.');
        }
        ...
        if (!onBehalf && notice > 0 && givenNotice < notice) {
            throw new UnprocessableError(`${type.name} needs ${notice} days' notice and this gives ${givenNotice}. Someone holding hr.leave_approve can record it for you if the notice cannot be met.`);
        }
        const clash = await trx
            ...
            .executeTakeFirst();
        if (clash) {
            throw new ConflictError(`Request ${clash.id} already covers ${String(clash.from_date)} to ${String(clash.to_date)} and is ${clash.status}. Withdraw it before raising another over the same days.`);
        }
        if (input.handoverToEmployeeId !== null) {
            ...
            const handover = await trx
                ...
                .executeTakeFirst();
            if (!handover)
                throw new UnprocessableError('That handover employee no longer exists.');
            if (handover.status === 'exited')
                throw new UnprocessableError('That handover employee has exited.');
        }
 ...
 */
function assertWithinQuota(bal, days, fy) {
    if (bal.quota === null)
        return;
    ...
    throw new UnprocessableError(`${bal.typeName} has ${dayCount(Math.round(available * 10) / 10)} available in ${fy} against a quota of ${dayCount(bal.quota)}, and this request needs ${dayCount(days)}. It is short by ${dayCount(shortfall)}.`);
}
/**
 ...
 */
export async function decideLeave(db, actor, requestId, input, opts) {
    return db.transaction().execute(async (trx) => {
        ...
        if (opts.approverEmployeeId !== null && Number(request.employee_id) === opts.approverEmployeeId) {
            throw new ForbiddenError('This is your own leave, so you cannot approve it. Someone else holding hr.leave_approve has to.');
        }
 ...
 * self-service action.
 */
export async function withdrawLeave(db, actor, requestId, opts) {
    await db.transaction().execute(async (trx) => {
        const request = await trx
            .selectFrom('leave_requests')
...
export async function createContractor(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        const clash = await trx
 ...
 */
export async function updateContractor(db, actor, contractorId, input) {
    await db.transaction().execute(async (trx) => {
        const before = await trx
            .selectFrom('labour_contractors')
        ...
        await writeAudit(trx, {
            ...
            before: {
                ...
                wc_policy_valid_until: before.wc_policy_valid_until === null ? null : String(before.wc_policy_valid_until),
            },
            after: {
                code: input.code,
                name: input.name,
 ...
 */
export async function addContractorRate(db, actor, contractorId, input) {
    return db.transaction().execute(async (trx) => {
        ...
        const existing = await trx
            ...
            .where('uom', '=', input.uom)
            .where((eb) => input.skillLevel === null
            ? eb('skill_level', 'is', null)
            : eb('skill_level', '=', input.skillLevel))
            .where((eb) => input.projectId === null ? eb('project_id', 'is', null) : eb('project_id', '=', input.projectId))
            .forUpdate()
            .execute();
        const duplicate = existing.find((r) => String(r.effective_from) === input.effectiveFrom);
        if (duplicate) {
            throw new ConflictError(`A ${input.workType} rate for that scope already starts on ${input.effectiveFrom}. Edit the period rather than adding a second line for the same day.`);
        }
 ...
 */
function complianceFailures(contractor, onDate) {
    const reasons = [];
 ...
 */
export async function recordContractorAttendance(db, actor, input, opts) {
    if (input.attendanceDate > today()) {
        throw new UnprocessableError('That date has not happened yet. Attendance is recorded for a day that has passed.');
    }
    return db.transaction().execute(async (trx) => {
        const contractor = await trx
            ...
            .executeTakeFirst();
        if (!contractor)
            throw new UnprocessableError('That contractor no longer exists.');
        if (contractor.status === 'blacklisted') {
            throw new UnprocessableError(`${contractor.name} is blacklisted, so no labour can be recorded against them. Change the status on the contractor record first -- there is no override for this one.`);
        }
        const project = await trx
            .selectFrom('projects')
            .select(['id', 'code'])
        ...
        if (failures.length > 0) {
            if (!input.overrideCompliance) {
                throw new UnprocessableError(`${contractor.name} cannot be given labour on ${input.attendanceDate}: ${failures.join(' and ')}. Someone holding hr.labour_contractor_manage can override this, and the override is recorded.`);
            }
            if (!opts.canManageContractors) {
                throw new ForbiddenError(`Overriding a compliance failure needs hr.labour_contractor_manage. Unresolved: ${failures.join(' and ')}.`);
            }
        }
        const priorRows = await trx
        ...
        const lineKey = (skillLevel, workType) => JSON.stringify([skillLevel, workType]);
        const priorByLine = new Map(priorRows.map((r) => [lineKey(String(r.skill_level), String(r.work_type)), r]));
        ...
        for (const row of input.rows) {
            ...
            if (held !== undefined && held !== basis) {
                ...
                throw new UnprocessableError(`${readable} is already on ${held === 'day' ? 'a day rate' : 'a measured rate'} for ${input.attendanceDate}, so the same skill level cannot also be put ${basis === 'day' ? 'on a day rate' : `on ${row.workType}`} that date. If those are the same people, the rate already on the day pays for the work; if they are two gangs, record the second under its own skill level or on its own date.`);
            }
        ...
        for (const row of input.rows) {
            ...
            if (prior && prior.bill_id !== null) {
                ...
                const billed = await trx
                    ...
                    .executeTakeFirst();
                throw new ConflictError(`The ${readable} row for ${input.attendanceDate} is already on bill ${billed?.bill_no ?? `#${Number(prior.bill_id)}`} and cannot be changed. Correcting a billed day is a finance adjustment.`);
            }
            ...
            if (measured && row.uom !== 'lumpsum' && (row.quantity === null || !(row.quantity > 0))) {
                throw new UnprocessableError(`The ${readable} line is quoted ${unit}, so it needs a quantity above zero to price. ${row.quantity === null ? 'None was given' : `${row.quantity} was given`}; a headcount alone cannot be multiplied by a ${unit} rate.`);
            }
            ...
            if (measured && row.workType === '') {
                throw new UnprocessableError(`The ${readable} line is quoted ${unit} and does not say what work it is for, so the rate card line that prices it cannot be identified.`);
            }
            if (!measured && row.workType !== '') {
                ...
                throw new UnprocessableError(`The ${readable} line is priced per day, which the skill level decides, so it cannot also name '${row.workType}' as its work. Clear the work type, or quote the line in the unit that work is measured in so the measure prices it.`);
            }
            if (!measured && row.quantity !== null) {
                throw new UnprocessableError(`The ${readable} line is quoted per day, which is priced by headcount, so it takes no quantity. Remove the ${row.quantity}, or quote the line in the unit that measure belongs to.`);
            }
            ...
            if (row.uom === 'lumpsum' && row.quantity !== 1) {
                throw new UnprocessableError(row.quantity === null
            ...
            if (!rate) {
                throw new UnprocessableError(`${contractor.name} has no ${unit} ${readable} rate${row.workType === '' ? '' : ` for ${row.workType}`} effective on ${input.attendanceDate}. Add it to the rate card before recording the day.`);
            }
            if (rate.ambiguous)
                ambiguousRates.push(`${readable}: rate ${rate.id} (${rate.workType})`);
            ...
            const amount = measured
                ? roundPaise(rate.ratePaise * row.quantity)
        ...
        await writeAudit(trx, {
            ...
            after: {
                ...
                rows: input.rows.map((r) => r.uom === 'per_day'
 ...
 */
export async function approveContractorAttendance(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        const rows = await trx
            ...
            .execute();
        if (rows.length === 0) {
            throw new UnprocessableError(`No contractor attendance is recorded for that project between ${input.from} and ${input.to}, so there is nothing to approve.`);
        }
        const pending = rows.filter((r) => r.approved_at === null);
        if (pending.length === 0) {
            throw new ConflictError(`All ${rows.length} ${rows.length === 1 ? 'row' : 'rows'} in that period are already approved.`);
        }
 ...
 */
export async function generateContractorBill(db, actor, input) {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (pending.length > 0) {
            ...
            throw new UnprocessableError(`${pending.length} ${pending.length === 1 ? 'row is' : 'rows are'} not approved yet, the earliest on ${earliest}. Rule 2 bills approved attendance only, and a bill that quietly left them out would never pick them up again. Approve or remove them first.`);
        }
        const billable = unbilled.filter((r) => r.approved_at !== null);
        if (billable.length === 0) {
            throw new UnprocessableError(`There is no approved unbilled attendance for ${contractor.name} on ${project.code} between ${input.from} and ${input.to}.`);
        }
        ...
        if (unpriceable.length > 0) {
            ...
            throw new UnprocessableError(`The ${String(first.skill_level).replace(/_/g, ' ')} row for ${String(first.attendance_date)} is quoted ${uomLabel(String(first.uom))} and carries no quantity, so its amount cannot be trusted. ${unpriceable.length === 1 ? 'It has' : `${unpriceable.length} rows in this period have`} to be re-entered before the period can be billed.`);
        }
        ...
        const netPayablePaise = grossPaise - input.advanceRecovered - retentionPaise - tdsPaise - input.penalty;
        if (netPayablePaise < 0) {
            throw new UnprocessableError(`The deductions come to more than the bill: ${formatPaiseAsRupees(grossPaise)} gross against ${formatPaiseAsRupees(input.advanceRecovered + retentionPaise + tdsPaise + input.penalty)} of advance, retention, TDS and penalty. A bill cannot be negative -- recover the balance on the next one.`);
        }
        ...
        if (Number(stamped.numUpdatedRows) !== ids.length) {
            throw new ConflictError('Some of those attendance rows were billed while this bill was being generated. Nothing has been saved -- reload and try again.');
        }
 ...
 */
export async function approveContractorBill(db, actor, billId, roleKeys) {
    return db.transaction().execute(async (trx) => {
        ...
        if (bill.status !== 'draft' && bill.status !== 'submitted' && bill.status !== 'verified') {
            throw new ConflictError(`Bill ${bill.bill_no} is ${bill.status}. Only a bill that has not been approved yet can be approved.`);
        }
        if (Number(bill.created_by) === actor.userId) {
            throw new UnprocessableError(`You generated bill ${bill.bill_no}, so you cannot approve it. Someone else holding the approval permission has to.`);
        }
        ...
        if (limit.requiresSecondApprovalAbove !== null && gross > limit.requiresSecondApprovalAbove) {
            ...
            throw new UnprocessableError(`${formatPaiseAsRupees(gross)} is above the ${formatPaiseAsRupees(limit.requiresSecondApprovalAbove)} single-approval threshold for your role, and contractor_bills has no column for a second approval. This bill cannot be approved until that is added.`);
        }
        await trx
