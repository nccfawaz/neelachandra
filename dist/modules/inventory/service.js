 ...
 */
export async function postStockMovement(trx, actor, m) {
  ...
  `.execute(trx);
    ...
    if (balanceAfter < 0 && m.txnType !== 'opening') {
        ...
        throw new UnprocessableError(`${store?.name ?? 'That store'} holds ${qty(qtyBefore)} ${unit} of ${named?.name ?? 'the item'}` +
...
export async function createItem(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        const clash = await trx
            ...
            .executeTakeFirst();
        if (clash)
            throw new ConflictError(`Item code ${input.code} is already in use.`);
        const row = await trx
            .insertInto('items')
            .values({
            code: input.code,
...
export async function updateItem(db, actor, itemId, input) {
 ...
 */
export async function addItemBrand(db, actor, itemId, input, canApprove) {
    return db.transaction().execute(async (trx) => {
        ...
        if (input.isApproved && !canApprove) {
            throw new UnprocessableError('Marking a brand approved needs the inventory.approve_po permission. Record it unapproved and ask for approval.');
        }
        const existing = await trx
            ...
            .executeTakeFirst();
        if (existing)
            throw new ConflictError(`${input.brand} is already listed against ${item.code}.`);
        const row = await trx
...
export async function setItemBrandApproval(db, actor, brandId, approved) {
    await db.transaction().execute(async (trx) => {
        const before = await trx
            .selectFrom('item_brands')
 ...
 * id is known.
 */
export async function createVendor(db, actor, input) {
    return db.transaction().execute(async (trx) => {
        const row = await trx
            .insertInto('vendors')
            .values({
...
export async function updateVendor(db, actor, vendorId, input) {
    await db.transaction().execute(async (trx) => {
        const before = await trx
            .selectFrom('vendors')
            .selectAll()
 ...
 */
export async function setVendorStatus(db, actor, vendorId, status, blacklistReason) {
    return db.transaction().execute(async (trx) => {
        const before = await trx
            .selectFrom('vendors')
...
export async function rateVendor(db, actor, vendorId, ratings) {
    await db.transaction().execute(async (trx) => {
        const before = await trx
            .selectFrom('vendors')
 ...
 */
export async function upsertVendorRate(db, actor, vendorId, input) {
    return db.transaction().execute(async (trx) => {
...
export async function createRequisition(db, actor, input) {
    return db.transaction().execute(async (trx) => {
 ...
 */
export async function submitRequisition(db, actor, requisitionId) {
    await db.transaction().execute(async (trx) => {
        const req = await trx
 ...
 */
export async function approveRequisition(db, actor, requisitionId, input) {
    await db.transaction().execute(async (trx) => {
        const req = await trx
            .selectFrom('material_requisitions')
        ...
        if (req.status !== 'submitted') {
            throw new UnprocessableError(`Only a submitted requisition can be approved. This one is ${req.status.replace(/_/g, ' ')}.`);
        }
        ...
        for (const line of input.lines) {
            ...
            if (!row)
                throw new UnprocessableError('That line is not on this requisition.');
            if (line.qtyApproved > Number(row.qty_requested)) {
                throw new UnprocessableError(`${row.code}: ${qty(line.qtyApproved)} cannot be approved against a request for ${qty(Number(row.qty_requested))}.`);
            }
            await trx
        ...
        if (!anyApproved) {
            throw new UnprocessableError('Every line was cut to zero. Reject the requisition instead, so the reason is on the record.');
        }
        await trx
            .updateTable('material_requisitions')
            .set({
...
export async function rejectRequisition(db, actor, requisitionId, reason) {
    await db.transaction().execute(async (trx) => {
        const req = await trx
            .selectFrom('material_requisitions')
            .select(['id', 'req_no', 'status', 'requested_by'])
        ...
        if (!req)
            throw new NotFoundError('Requisition not found');
        if (req.status !== 'submitted') {
            throw new UnprocessableError(`Only a submitted requisition can be rejected. This one is ${req.status.replace(/_/g, ' ')}.`);
        }
        await trx
            .updateTable('material_requisitions')
 ...
 */
function poTotals(lines, freightPaise) {
    let subtotal = 0;
    let gst = 0;
    const lineTotals = [];
...
export async function poRateWarnings(db, lines) {
    const out = [];
    for (const line of lines) {
        ...
        if (Math.abs(variance) <= RATE_VARIANCE_THRESHOLD_PCT)
            continue;
        out.push({
            itemId: line.itemId,
 ...
 */
export async function createPo(db, actor, input) {
    ...
    return await db.transaction().execute(async (trx) => {
        const vendor = await trx
            ...
            .executeTakeFirst();
        if (!vendor)
            throw new NotFoundError('That vendor does not exist.');
        if (vendor.status === 'blacklisted') {
            throw new UnprocessableError(`${vendor.name} is blacklisted. Remove the blacklist before ordering, so the decision to buy from them again is recorded as a decision.`);
        }
 ...
 */
async function linkRequisitionOrdered(trx, requisitionId) {
    ...
    for (const o of ordered)
        orderedByItem.set(Number(o.item_id), Number(o.qty));
...
export async function submitPo(db, actor, poId) {
    await db.transaction().execute(async (trx) => {
        const po = await trx
            .selectFrom('purchase_orders')
            ...
            .executeTakeFirst();
        if (!po)
            throw new NotFoundError('That purchase order does not exist.');
        if (po.status !== 'draft') {
 ...
 * prevent.
 */
export async function approvePo(db, actor, poId, roleKeys) {
    return await db.transaction().execute(async (trx) => {
        const po = await trx
            .selectFrom('purchase_orders')
            .select([
        ...
        if (po.status !== 'pending_approval') {
            throw new ConflictError(`This purchase order is ${po.status.replace(/_/g, ' ')}. Only one awaiting approval can be approved.`);
        }
        if (Number(po.created_by) === actor.userId) {
            throw new UnprocessableError('You raised this purchase order, so you cannot approve it. Someone else holding the approval permission has to.');
        }
        ...
        if (needsSecond && isFirst) {
            ...
            return { status: 'awaiting_second_approval', poNo: po.po_no, totalPaise: total };
        }
        await trx
            .updateTable('purchase_orders')
            .set(isFirst
        ...
        return { status: 'approved', poNo: po.po_no, totalPaise: total };
    });
}
/**
 ...
 */
export async function shortClosePo(db, actor, poId, reason) {
    ...
    await db.transaction().execute(async (trx) => {
        ...
        if (po.status === 'short_closed')
            throw new ConflictError('This purchase order is already short-closed.');
        if (po.status === 'cancelled')
            throw new ConflictError('This purchase order is cancelled.');
        if (po.status === 'received') {
            throw new UnprocessableError('This purchase order is fully received. There is no balance to short-close.');
        }
        if (po.status === 'draft' || po.status === 'pending_approval') {
            throw new UnprocessableError(`A ${po.status.replace(/_/g, ' ')} purchase order has nothing outstanding against it. Cancel it instead.`);
        }
        const balance = await trx
            .selectFrom('po_lines')
            .select([
            trx.fn.sum(sql `qty_ordered - qty_received`).as('outstanding'),
        ])
            .where('po_id', '=', poId)
            .executeTakeFirstOrThrow();
        await trx
 ...
 */
export async function createGrn(db, actor, input) {
    return await db.transaction().execute(async (trx) => {
        if (input.poId !== null) {
            const po = await trx
                .selectFrom('purchase_orders')
            ...
            if (Number(po.vendor_id) !== input.vendorId) {
                throw new UnprocessableError(`Purchase order ${po.po_no} is on a different vendor. Receive against the vendor who was ordered from, or record this as a direct receipt with no purchase order.`);
            }
            if (po.status === 'draft' || po.status === 'pending_approval') {
                throw new UnprocessableError(`Purchase order ${po.po_no} is not approved yet. Material received against an unapproved order is a direct receipt; either approve the order or leave the order field empty.`);
            }
            if (po.status === 'cancelled') {
 ...
 */
export async function postGrn(db, actor, grnId, canApproveBrand) {
    return await db.transaction().execute(async (trx) => {
        const grn = await trx
            ...
            .executeTakeFirst();
        if (!grn)
            throw new NotFoundError('That goods receipt does not exist.');
        if (grn.status === 'posted')
            throw new ConflictError('This goods receipt is already posted. Post a reversing adjustment to correct it.');
        if (grn.status === 'cancelled')
            throw new ConflictError('This goods receipt is cancelled.');
        const lines = await trx
            .selectFrom('grn_lines')
            .innerJoin('items', 'items.id', 'grn_lines.item_id')
            .innerJoin('units', 'units.id', 'items.unit_id')
        ...
        for (const l of lines) {
            ...
            if (Math.abs(challan - received) < 0.0005)
                continue;
            shortages.push({
            ...
            if ((l.rejection_reason ?? '').trim().length === 0) {
                unexplained.push(`${l.item_code} ${l.item_name}: challan says ${qty(challan)} ${l.unit}, counted ${qty(received)} ${l.unit}`);
            }
        }
        if (unexplained.length > 0) {
            throw new UnprocessableError(`Fill in a reason on every line where the challan and the counted quantity disagree, then post. Without it the shortage cannot be claimed from the vendor.\n\n${unexplained.join('\n')}`);
        }
        ...
        if (brandExceptions.length > 0 && !canApproveBrand) {
            throw new UnprocessableError(`These lines are on brands that are not approved for the item, so posting needs purchase approval rights. The packages page names the approved brands to clients, so a substitution has to be approved by someone who can carry that decision.\n\n${brandExceptions
        ...
        if (grn.po_id !== null) {
            ...
            for (const l of lines) {
                ...
                const posted = await trx
                    ...
                    .where((eb) => eb.or([eb('goods_receipts.status', '=', 'posted'), eb('goods_receipts.id', '=', grnId)]))
                    .executeTakeFirstOrThrow();
                await trx
            ...
            const balance = await trx
                .selectFrom('po_lines')
                .select([
                trx.fn.sum(sql `GREATEST(qty_ordered - qty_received, 0)`).as('outstanding'),
        ...
        const receivedValuePaise = lines.reduce((sum, l) => sum + Math.round(Number(l.qty_accepted) * Number(l.rate_paise)), 0);
        ...
        if (receivedValuePaise === 0) {
            throw new UnprocessableError(`Goods receipt ${grn.grn_no} has zero accepted value (every line's accepted quantity × rate is zero), so there is no cost to post. Correct the receipt lines before posting.`);
        }
 ...
 */
export function allocateBatches(available, wanted) {
    ...
    if (left > 0)
        picks.push({ batchNo: null, qty: left, expiryDate: null, isExpired: false });
    return picks;
}
/**
 ...
 */
export async function createIssue(db, actor, input) {
    return await db.transaction().execute(async (trx) => {
        const project = await trx
            .selectFrom('projects')
        ...
        for (const line of input.lines) {
            const item = await trx
                ...
                .executeTakeFirst();
            if (!item)
                throw new NotFoundError('One of the items on this issue does not exist.');
            if (item.is_active === 0) {
            ...
            if (line.batchNo !== null) {
                ...
                picks = allocateBatches(balances.map((b) => ({
                    batchNo: b.batchNo,
                    qty: b.qty,
            ...
            const issuedValue = await trx
                ...
                .select([
                trx.fn.sum(sql `qty_out`).as('qty'),
                trx.fn.sum(sql `value_paise`).as('value'),
            ])
 ...
 */
export async function returnIssue(db, actor, issueId, returnedOn, lines) {
    await db.transaction().execute(async (trx) => {
        const issue = await trx
            ...
            .executeTakeFirst();
        if (!issue)
            throw new NotFoundError('That issue does not exist.');
        if (issue.status === 'cancelled')
            throw new ConflictError('That issue is cancelled.');
        for (const line of lines) {
            const il = await trx
                ...
                .executeTakeFirst();
            if (!il)
                throw new NotFoundError('One of the lines being returned is not on that issue.');
            ...
            if (line.qtyReturned > outstanding + 0.0005) {
                throw new UnprocessableError(`${il.item_code} ${il.item_name}: ${qty(line.qtyReturned)} ${il.unit} cannot come back when only ${qty(outstanding)} ${il.unit} is still out` +
 ...
 */
async function ensureTransitLocation(trx) {
    const existing = await trx
        .selectFrom('locations')
        .select('id')
 ...
 */
export async function dispatchTransfer(db, actor, input) {
    ...
    return await db.transaction().execute(async (trx) => {
        ...
        if (!from || !to)
            throw new NotFoundError('One of those stores does not exist.');
        if (to.is_active === 0) {
 ...
 */
export async function receiveTransfer(db, actor, transferId, receivedOn, lines) {
    return await db.transaction().execute(async (trx) => {
        ...
        if (transfer.status === 'received')
            throw new ConflictError('That transfer is already received.');
        if (transfer.status === 'cancelled')
            throw new ConflictError('That transfer is cancelled.');
        if (receivedOn < String(transfer.dispatched_on)) {
            throw new UnprocessableError(`The receipt date is before the dispatch date of ${formatDate(String(transfer.dispatched_on))}.`);
        }
        const transitId = await ensureTransitLocation(trx);
        const shortages = [];
        for (const line of lines) {
            const tl = await trx
                ...
                .executeTakeFirst();
            if (!tl)
                throw new NotFoundError('One of the lines being received is not on that transfer.');
            const sent = Number(tl.qty_sent);
            if (line.qtyReceived > sent + 0.0005) {
                throw new UnprocessableError(`${tl.item_code} ${tl.item_name}: ${qty(line.qtyReceived)} ${tl.unit} cannot be received when only ${qty(sent)} ${tl.unit} was sent. Receive what arrived and post an adjustment for the difference, so the extra has a reason on it.`);
            }
 ...
 */
export async function postAdjustment(db, actor, input) {
    return await db.transaction().execute(async (trx) => {
        const location = await trx
            ...
            .executeTakeFirst();
        if (!location)
            throw new NotFoundError('That store does not exist.');
        const inserted = await trx
        ...
        for (const line of input.lines) {
            const item = await trx
                ...
                .executeTakeFirst();
            if (!item)
                throw new NotFoundError('One of the counted items does not exist.');
            const cached = await trx
                .selectFrom('item_stock')
                .select(['qty_on_hand', 'value_paise'])
                .where('item_id', '=', line.itemId)
 ...
 */
export async function postOpeningStock(db, actor, input) {
    return await db.transaction().execute(async (trx) => {
        const location = await trx
            .selectFrom('locations')
            .select(['id', 'name'])
        ...
        if (Number(prior?.n ?? 0) > 0) {
            throw new ConflictError(`${item.code} ${item.name} already has stock movements at ${location.name}. ` +
 ...
 */
export async function deployEquipment(db, actor, equipmentId, input) {
    return await db.transaction().execute(async (trx) => {
        const eq = await trx
            ...
            .executeTakeFirst();
        if (!eq)
            throw new NotFoundError('That equipment does not exist.');
        if (eq.status === 'retired')
            throw new ConflictError(`${eq.code} ${eq.name} is retired.`);
        ...
        const open = await trx
            ...
            .executeTakeFirst();
        if (open) {
            throw new ConflictError(`${eq.code} ${eq.name} is already on ${open.code} ${open.name} since ${formatDate(String(open.from_date))}. ` +
 ...
 */
export async function returnEquipment(db, actor, equipmentId, input) {
    return await db.transaction().execute(async (trx) => {
        const eq = await trx
            .selectFrom('equipment')
        ...
        const open = await trx
            ...
            .executeTakeFirst();
        if (!open)
            throw new ConflictError(`${eq.code} ${eq.name} is not deployed anywhere.`);
        const fromDate = String(open.from_date).slice(0, 10);
        if (input.toDate < fromDate) {
            throw new UnprocessableError(`The return date is before the deployment date, ${formatDate(fromDate)}. Check the date on the log sheet.`);
        }
        ...
        if (input.meterEnd !== null && meterStart !== null && input.meterEnd < meterStart) {
            throw new UnprocessableError(`The closing meter reading (${input.meterEnd}) is below the opening reading (${meterStart}). ` +
        ...
        if (input.locationId !== null) {
            const dest = await trx
                ...
                .executeTakeFirst();
            if (!dest)
                throw new NotFoundError('That yard or store does not exist.');
            if (dest.is_active === 0)
                throw new UnprocessableError('That yard or store is inactive.');
        }
        await trx
        ...
        const hireCostPaise = eq.ownership === 'hired' && eq.hire_rate_per_day_paise !== null
 ...
 */
export async function stockAlerts(db, scope, opts = {}) {
 ...
 */
export async function getConsumptionVariance(db, projectId) {
    ...
    const issued = await db
        ...
        'units.code as unit',
        sql `SUM(stock_ledger.qty_out - stock_ledger.qty_in)`.as('qty_net'),
        sql `SUM(CASE WHEN stock_ledger.qty_out > 0
    ...
    for (const n of norms)
        normByItem.set(Number(n.item_id), Number(n.qty_per_sqft));
    const lines = issued.map((r) => {
        ...
        let expectedAtCompletion = null;
        let expectedAtProgress = null;
        let pct = null;
        let excessQty = null;
        let excessValuePaise = null;
        let flag = 'no_norm';
        if (norm === null) {
            flag = 'no_norm';
        }
    ...
    const notes = [];
    if (norms.length === 0) {
        notes.push('No consumption norms are set for this project type, so nothing can be compared yet. ' +
    ...
    if (project.delivery_model !== 'package_per_sqft') {
        notes.push(`The per-sqft norm is specified for package_per_sqft projects; this one is ${project.delivery_model}. ` +
            'Read the comparison as indicative.');
    }
    if (progressPct <= 0) {
        notes.push('Physical progress is nil, so the pro-rated expectation is nil and every issued item reads as over-consumption. ' +
