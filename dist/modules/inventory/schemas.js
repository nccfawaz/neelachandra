 ...
 */
const rawList = z.unknown().transform((raw) => {
...
function textAt(col, i, max) {
    const v = (col[i] ?? '').slice(0, max);
    return v === '' ? null : v;
}
function dateAt(col, i) {
    const v = col[i] ?? '';
    return /^\d{4}-\d{2}-\d{2}$/.test(v) ? v : null;
}
function badLine(ctx, i, message) {
 ...
 */
const yesNo = (fallback) => z
    .string()
    .optional()
    .transform((v) => (v === undefined || v === '' ? fallback : v === '1' || v === 'true' || v === 'on'));
export const VENDOR_TYPES = ['material', 'equipment_hire', 'subcontractor', 'service', 'transport'];
export const VENDOR_STATUSES = ['active', 'on_hold', 'blacklisted'];
export const ISSUED_TO_TYPES = ['own_labour', 'labour_contractor', 'subcontractor'];
...
export const vendorStatusSchema = z
    ...
    .refine((v) => v.status !== 'blacklisted' || v.blacklistReason !== null, 'Blacklisting a vendor needs a reason on the record.');
...
export const requisitionSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one item line.' });
    return {
...
export const requisitionApproveSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Nothing to approve.' });
...
export const poSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one order line.' });
    return {
        vendorId: v.vendorId,
        projectId: v.projectId,
...
export const grnSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one receipt line.' });
    return {
        poId: v.poId,
        vendorId: v.vendorId,
...
export const issueSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one issue line.' });
...
export const issueReturnSchema = z
    ...
    for (let i = 0; i < v.issueLineId.length; i += 1) {
        ...
        if (qty > 0)
            lines.push({ issueLineId, qtyReturned: qty });
    }
    if (lines.length === 0) {
...
export const transferSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one transfer line.' });
...
export const transferReceiveSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Nothing to receive.' });
...
export const adjustmentSchema = z
    ...
    if (lines.length === 0)
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Add at least one counted item.' });
    return {
        locationId: v.locationId,
