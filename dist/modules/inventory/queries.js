...
function scopeIn(ids) {
    return ids.length ? ids : [0];
}
export async function stockRows(db, scope, opts) {
    ...
    if (opts.canViewRates)
        query = query.select('item_stock.value_paise');
    if (locations)
        query = query.where('item_stock.location_id', 'in', scopeIn(locations));
    if (opts.locationId)
        query = query.where('item_stock.location_id', '=', opts.locationId);
    if (opts.categoryId)
        query = query.where('items.category_id', '=', opts.categoryId);
    if (opts.q) {
        const like = `%${opts.q}%`;
    ...
    if (opts.belowReorder) {
        query = query
            ...
            .where(sql `item_stock.qty_on_hand <= items.reorder_level`);
    }
    return (await query.execute());
}
export async function stockRowCount(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db
        .selectFrom('item_stock')
        .innerJoin('items', 'items.id', 'item_stock.item_id')
...
export async function stockSummary(db, scope, canViewRates) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db
        .selectFrom('item_stock')
        .select((eb) => [
        eb.fn.count('item_stock.item_id').distinct().as('items'),
        eb.fn.count('item_stock.location_id').distinct().as('locations'),
        eb.fn.sum('item_stock.value_paise').as('value_paise'),
    ])
...
export async function lowStock(db, scope, limit = 50) {
 ...
 */
export async function batchBalances(db, itemId, locationId) {
    const rows = await db
        .selectFrom('stock_ledger')
        .leftJoin('grn_lines', (join) => join.onRef('grn_lines.batch_no', '=', 'stock_ledger.batch_no').onRef('grn_lines.item_id', '=', 'stock_ledger.item_id'))
        .select([
        'stock_ledger.batch_no',
        sql `SUM(stock_ledger.qty_in - stock_ledger.qty_out)`.as('qty'),
        sql `MAX(grn_lines.expiry_date)`.as('expiry_date'),
    ])
 ...
 */
export async function expiringBatches(db, withinDays = 30, onDate = today()) {
    const rows = await db
        ...
        .where('grn_lines.expiry_date', '<=', sql `DATE_ADD(${onDate}, INTERVAL ${sql.lit(withinDays)} DAY)`)
...
export async function itemLedger(db, opts) {
    let query = db
        .selectFrom('stock_ledger')
    ...
    if (opts.canViewRates)
        query = query.select(['stock_ledger.rate_paise', 'stock_ledger.value_paise']);
    if (opts.locationId)
        query = query.where('stock_ledger.location_id', '=', opts.locationId);
    return query.execute();
}
export async function listItems(db, opts) {
    let query = db
        .selectFrom('items')
        .innerJoin('units', 'units.id', 'items.unit_id')
        .innerJoin('item_categories', 'item_categories.id', 'items.category_id')
        ...
        .offset(opts.offset ?? 0);
    if (opts.activeOnly)
        query = query.where('items.is_active', '=', 1);
    if (opts.categoryId)
        query = query.where('items.category_id', '=', opts.categoryId);
    if (opts.q) {
        const like = `%${opts.q}%`;
...
export async function countItems(db, opts) {
    let query = db.selectFrom('items').select((eb) => eb.fn.countAll().as('n'));
    if (opts.activeOnly)
        query = query.where('items.is_active', '=', 1);
    if (opts.categoryId)
        query = query.where('items.category_id', '=', opts.categoryId);
...
export async function findItem(db, id) {
    return db
        .selectFrom('items')
        .innerJoin('units', 'units.id', 'items.unit_id')
        .innerJoin('item_categories', 'item_categories.id', 'items.category_id')
...
export async function itemBrandRows(db, itemId) {
    return db
...
export async function itemOptions(db) {
    return db
        .selectFrom('items')
        .innerJoin('units', 'units.id', 'items.unit_id')
...
export async function categoryOptions(db) {
    return db
...
export async function locationOptions(db, scope, types) {
    ...
    if (types && types.length) {
        query = query.where('locations.location_type', 'in', types);
    }
    if (allowed)
        query = query.where('locations.id', 'in', scopeIn(allowed));
    return query.execute();
}
export async function listRequisitions(db, scope, opts) {
    ...
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.status)
        query = query.where('mr.status', '=', opts.status);
    if (opts.projectId)
        query = query.where('mr.project_id', '=', opts.projectId);
    if (scoped)
        query = query.where('mr.project_id', 'in', scopeIn(scoped));
    return query.execute();
}
export async function countRequisitions(db, scope, opts) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db.selectFrom('material_requisitions').select((eb) => eb.fn.countAll().as('n'));
    if (opts.status)
        query = query.where('status', '=', opts.status);
    if (opts.projectId)
        query = query.where('project_id', '=', opts.projectId);
    if (scoped)
        query = query.where('project_id', 'in', scopeIn(scoped));
    const row = await query.executeTakeFirst();
    return Number(row?.n ?? 0);
}
export async function findRequisition(db, id) {
    return db
        .selectFrom('material_requisitions as mr')
        .innerJoin('projects', 'projects.id', 'mr.project_id')
...
export async function requisitionLineRows(db, requisitionId) {
    return db
...
export async function orderableRequisitionLines(db, requisitionId) {
    const rows = await db
        .selectFrom('requisition_lines as rl')
        .innerJoin('items', 'items.id', 'rl.item_id')
        .innerJoin('units', 'units.id', 'items.unit_id')
...
export async function listPurchaseOrders(db, scope, opts) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.canViewRates)
        query = query.select(['po.total_paise', 'po.subtotal_paise', 'po.gst_paise']);
    if (opts.status)
        query = query.where('po.status', '=', opts.status);
    if (opts.vendorId)
        query = query.where('po.vendor_id', '=', opts.vendorId);
    if (scoped) {
        query = query.where((eb) => eb.or([eb('po.project_id', 'is', null), eb('po.project_id', 'in', scopeIn(scoped))]));
    }
    return query.execute();
}
export async function countPurchaseOrders(db, scope, opts) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db.selectFrom('purchase_orders').select((eb) => eb.fn.countAll().as('n'));
    if (opts.status)
        query = query.where('status', '=', opts.status);
    if (opts.vendorId)
...
export async function findPurchaseOrder(db, id, canViewRates) {
    let query = db
        .selectFrom('purchase_orders as po')
        .innerJoin('vendors', 'vendors.id', 'po.vendor_id')
...
export async function poLineRows(db, poId, canViewRates) {
    let query = db
        ...
        .orderBy('pl.id');
    if (canViewRates)
        query = query.select(['pl.rate_paise', 'pl.line_total_paise']);
    return query.execute();
}
/** PO lines with quantity outstanding, for GRN prefill. */
export async function receivablePoLines(db, poId) {
    const rows = await db
        .selectFrom('po_lines as pl')
        .innerJoin('items', 'items.id', 'pl.item_id')
        .innerJoin('units', 'units.id', 'items.unit_id')
 ...
 */
export async function rateReference(db, itemId, vendorId, onDate = today()) {
    let ratePaise = null;
    if (vendorId) {
        const row = await db
...
export async function listGrns(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db
        ...
        eb
            ...
            .select(sql `COUNT(*)`.as('c'))
            .whereRef('grn_lines.grn_id', '=', 'gr.id')
            .where(sql `grn_lines.qty_challan <> grn_lines.qty_received`)
            .as('mismatch_count'),
    ])
        .orderBy('gr.id', 'desc')
        .limit(opts.limit ?? PAGE_SIZE)
        .offset(opts.offset ?? 0);
    if (opts.status)
        query = query.where('gr.status', '=', opts.status);
    if (opts.locationId)
        query = query.where('gr.location_id', '=', opts.locationId);
    if (locations)
        query = query.where('gr.location_id', 'in', scopeIn(locations));
    return query.execute();
}
export async function countGrns(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db.selectFrom('goods_receipts').select((eb) => eb.fn.countAll().as('n'));
    if (opts.status)
        query = query.where('status', '=', opts.status);
    if (opts.locationId)
        query = query.where('location_id', '=', opts.locationId);
    if (locations)
        query = query.where('location_id', 'in', scopeIn(locations));
    const row = await query.executeTakeFirst();
    return Number(row?.n ?? 0);
}
export async function findGrn(db, id) {
    return db
        .selectFrom('goods_receipts as gr')
        .innerJoin('vendors', 'vendors.id', 'gr.vendor_id')
        .innerJoin('locations', 'locations.id', 'gr.location_id')
...
export async function grnLineRows(db, grnId, canViewRates) {
    let query = db
        .selectFrom('grn_lines as gl')
        ...
        .orderBy('gl.id');
    if (canViewRates)
        query = query.select('gl.rate_paise');
    return query.execute();
}
export async function listIssues(db, scope, opts) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.projectId)
        query = query.where('mi.project_id', '=', opts.projectId);
    if (opts.locationId)
        query = query.where('mi.location_id', '=', opts.locationId);
    if (scoped)
        query = query.where('mi.project_id', 'in', scopeIn(scoped));
    return query.execute();
}
export async function countIssues(db, scope, opts) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db.selectFrom('material_issues').select((eb) => eb.fn.countAll().as('n'));
    if (opts.projectId)
        query = query.where('project_id', '=', opts.projectId);
    if (opts.locationId)
        query = query.where('location_id', '=', opts.locationId);
...
export async function findIssue(db, id) {
    return db
        .selectFrom('material_issues as mi')
        .innerJoin('projects', 'projects.id', 'mi.project_id')
        .innerJoin('locations', 'locations.id', 'mi.location_id')
...
export async function issueLineRows(db, issueId, canViewRates) {
    let query = db
        ...
        .orderBy('il.id');
    if (canViewRates)
        query = query.select('il.rate_paise');
    return query.execute();
}
export async function listTransfers(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.status)
        query = query.where('st.status', '=', opts.status);
    if (locations) {
        query = query.where((eb) => eb.or([eb('st.from_location_id', 'in', scopeIn(locations)), eb('st.to_location_id', 'in', scopeIn(locations))]));
    }
    return query.execute();
}
export async function countTransfers(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db.selectFrom('stock_transfers').select((eb) => eb.fn.countAll().as('n'));
    if (opts.status)
        query = query.where('status', '=', opts.status);
    if (locations) {
        query = query.where((eb) => eb.or([eb('from_location_id', 'in', scopeIn(locations)), eb('to_location_id', 'in', scopeIn(locations))]));
    }
    const row = await query.executeTakeFirst();
    return Number(row?.n ?? 0);
}
export async function findTransfer(db, id) {
    return db
        .selectFrom('stock_transfers as st')
        .innerJoin('locations as fl', 'fl.id', 'st.from_location_id')
        .innerJoin('locations as tl', 'tl.id', 'st.to_location_id')
...
export async function transferLineRows(db, transferId) {
    return db
...
export async function listAdjustments(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.reason)
        query = query.where('sa.reason', '=', opts.reason);
    if (locations)
        query = query.where('sa.location_id', 'in', scopeIn(locations));
    return query.execute();
}
export async function countAdjustments(db, scope, opts) {
    const locations = await accessibleLocationIds(db, scope);
    let query = db.selectFrom('stock_adjustments').select((eb) => eb.fn.countAll().as('n'));
    if (opts.reason)
...
export async function findAdjustment(db, id) {
    return db
        .selectFrom('stock_adjustments as sa')
        .innerJoin('locations', 'locations.id', 'sa.location_id')
        .leftJoin('users as cre', 'cre.id', 'sa.created_by')
...
export async function adjustmentLineRows(db, adjustmentId, canViewRates) {
    let query = db
        ...
        .orderBy('al.id');
    if (canViewRates)
        query = query.select('al.rate_paise');
    return query.execute();
}
export async function listVendors(db, opts) {
    let query = db
        .selectFrom('vendors')
        .select([
        'id',
        ...
        .offset(opts.offset ?? 0);
    if (opts.vendorType)
        query = query.where('vendor_type', '=', opts.vendorType);
    if (opts.status)
        query = query.where('status', '=', opts.status);
    if (opts.q) {
        const like = `%${opts.q}%`;
        query = query.where((eb) => eb.or([eb('name', 'like', like), eb('code', 'like', like), eb('gstin', 'like', like)]));
    }
    return query.execute();
}
export async function countVendors(db, opts) {
    let query = db.selectFrom('vendors').select((eb) => eb.fn.countAll().as('n'));
    if (opts.vendorType)
        query = query.where('vendor_type', '=', opts.vendorType);
    if (opts.status)
        query = query.where('status', '=', opts.status);
...
export async function findVendor(db, id) {
    return db.selectFrom('vendors').selectAll().where('id', '=', id).executeTakeFirst();
}
export async function vendorOptions(db, vendorType) {
    let query = db
        .selectFrom('vendors')
...
export async function vendorRateRows(db, vendorId) {
    return db
...
export async function vendorPurchaseHistory(db, vendorId, canViewRates, limit = 20) {
    let query = db
        .selectFrom('purchase_orders as po')
        .leftJoin('projects', 'projects.id', 'po.project_id')
        ...
        .limit(limit);
    if (canViewRates)
        query = query.select('po.total_paise');
    return query.execute();
}
/**
 ...
 */
export async function consumptionForProject(db, projectId) {
    ...
    const rows = await db
        ...
        'items.wastage_allowance_pct',
        'units.code as unit_code',
        sql `SUM(il.qty_issued - il.qty_returned)`.as('issued_qty'),
    ])
    ...
    return rows.map((r) => {
        ...
        return {
            ...
            variancePct: expected === null || expected === 0 ? null : Math.round(((issuedQty - expected) / expected) * 1000) / 10,
        };
    });
}
export async function listEquipment(db, opts) {
    let query = db
        ...
        .offset(opts.offset ?? 0);
    if (opts.canViewRates)
        query = query.select('eq.hire_rate_per_day_paise');
    if (opts.status)
        query = query.where('eq.status', '=', opts.status);
    return query.execute();
}
export async function countEquipment(db, opts) {
    let query = db.selectFrom('equipment').select((eb) => eb.fn.countAll().as('n'));
    if (opts.status)
        query = query.where('status', '=', opts.status);
    const row = await query.executeTakeFirst();
    return Number(row?.n ?? 0);
}
export async function findEquipment(db, id) {
    return db
        .selectFrom('equipment as eq')
        .leftJoin('locations', 'locations.id', 'eq.current_location_id')
        .leftJoin('projects', 'projects.id', 'eq.current_project_id')
...
export async function equipmentDeploymentRows(db, equipmentId, limit = 20) {
    return db
 ...
 */
export async function equipmentDue(db, withinDays = 30, onDate = today()) {
    const horizon = sql `DATE_ADD(${onDate}, INTERVAL ${sql.lit(withinDays)} DAY)`;
    const rows = await db
        .selectFrom('equipment as eq')
        .leftJoin('projects', 'projects.id', 'eq.current_project_id')
        .select([
        ...
        .where((eb) => eb.or([eb('eq.next_service_due', '<=', horizon), eb('eq.insurance_valid_until', '<=', horizon)]))
        .orderBy('eq.code')
        .execute();
    return rows.map((r) => ({
...
export async function inventoryCounts(db, scope) {
    const scoped = await projectScopeFilter(db, scope);
    let req = db
...
export async function projectOptions(db, scope) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db
        .selectFrom('projects')
        ...
        .orderBy('code');
    if (scoped)
        query = query.where('id', 'in', scopeIn(scoped));
    return query.execute();
}
export async function projectStageOptions(db, projectId) {
    return db
...
export async function labourContractorOptions(db) {
    return db
...
export async function inspectorOptions(db) {
    return db
        .selectFrom('users')
        .select(['id', 'full_name'])
        .where('status', '=', 'active')
