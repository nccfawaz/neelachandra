 ...
 * against stages.
 */
export async function recalcProjectProgress(trx, projectId) {
    const row = await trx
        .selectFrom('project_stages')
        .select((eb) => [
        eb.fn
            .sum(eb(eb.ref('progress_pct'), '*', eb.ref('weightage_pct')))
            .as('weighted'),
        eb.fn.sum('weightage_pct').as('weight'),
    ])
 ...
 */
export async function setStageProgress(db, actor, opts) {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (opts.progressPct > 0 && stage.predecessor_stage_id) {
            ...
            if (pred && Number(pred.progress_pct) < 100) {
                if (!opts.override || opts.override.trim().length < 10) {
                    throw new UnprocessableError(`${pred.name} is at ${Number(pred.progress_pct)}% and must finish first. To start anyway, supply an override reason of at least 10 characters.`);
                }
                if (!opts.canOverride) {
                    throw new UnprocessableError(`${pred.name} is not complete. Overriding the sequence needs the projects.manage permission.`);
                }
        ...
        const status = opts.progressPct >= 100 ? 'complete' : opts.progressPct > 0 ? 'in_progress' : 'not_started';
        await trx
 ...
 * finished.
 */
export async function createProject(db, actor, input) {
    return db.transaction().execute(async (trx) => {
 ...
 */
export async function resolveStageTemplate(trx, projectType, templateId) {
    if (templateId)
        return templateId;
    const tpl = await trx
 ...
 */
export async function instantiateStagesFromTemplate(trx, projectId, templateId) {
    if (!templateId)
        return 0;
    ...
    if (Math.abs(sum - 100) > 0.01) {
        throw new UnprocessableError(`The stage template weightages sum to ${sum.toFixed(2)} rather than 100. Correct the template before creating a project from it.`);
    }
    const stageIds = [];
...
export async function ensureSiteStore(trx, projectId, projectCode, projectName, city, address) {
 ...
 */
export async function setProjectStatus(db, actor, projectId, next, reason) {
    await db.transaction().execute(async (trx) => {
        ...
        if (!ALLOWED_TRANSITIONS[current].includes(next)) {
            throw new UnprocessableError(`A project cannot move from ${current.replace(/_/g, ' ')} to ${next.replace(/_/g, ' ')}. Allowed: ${ALLOWED_TRANSITIONS[current].map((s) => s.replace(/_/g, ' ')).join(', ') || 'none'}.`);
        }
        ...
        if (next === 'handed_over') {
            ...
            if (Number(blocking?.n ?? 0) > 0) {
                throw new UnprocessableError(`${Number(blocking?.n)} structural or safety snag(s) are still open. Handover with an open safety defect transfers a live hazard to the client.`);
            }
        }
        if (next === 'closed') {
            ...
            if (Number(stock?.n ?? 0) > 0) {
                throw new UnprocessableError(`${Number(stock?.n)} item(s) still show stock at this site store. Return or transfer them before closing, or the stock becomes phantom inventory nobody owns.`);
            }
        }
        const patch = {
            status: next,
        ...
        if (next === 'in_progress' && current === 'mobilising')
 ...
 */
export async function submitDpr(db, actor, projectId, input) {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (existing?.reviewed_at) {
            throw new ConflictError('That day has already been reviewed. Ask a project manager to reopen it rather than overwriting a signed-off report.');
        }
        const values = {
            project_id: projectId,
            ...
            stoppage_reason: input.stoppageReason,
        ...
        for (const sp of input.stageProgress) {
            ...
            if (!stage)
                continue;
            if (sp.pct < Number(stage.progress_pct))
                continue;
            await trx
                .updateTable('project_stages')
                .set({
        ...
        if (input.stageProgress.length > 0)
            await recalcProjectProgress(trx, projectId);
        await writeAudit(trx, {
            userId: actor.userId,
...
export async function reviewDpr(db, actor, dprId) {
    await db.transaction().execute(async (trx) => {
        const dpr = await trx
            .selectFrom('daily_progress_reports')
...
/* Quality ---------------------------------------------------------------- */
export async function createQualityCheck(db, actor, projectId, input) {
    return db.transaction().execute(async (trx) => {
        const row = await trx
...
export async function signOffQualityCheck(db, actor, checkId) {
    await db.transaction().execute(async (trx) => {
        ...
        if (check.signed_off_by)
            throw new ConflictError('That check has already been signed off.');
 ...
 */
export async function certifyMilestone(db, actor, projectId, milestoneId) {
    await db.transaction().execute(async (trx) => {
        const ms = await trx
        ...
        if (ms.trigger_stage_id) {
            const stage = await trx
                ...
                .executeTakeFirst();
            if (!stage)
                throw new UnprocessableError('The trigger stage for this milestone is missing.');
            if (Number(stage.progress_pct) < 100) {
                throw new UnprocessableError(`${stage.name} is at ${Number(stage.progress_pct)}%. A milestone cannot be certified before the work that triggers it is complete.`);
            }
            ...
            if (failed.length > 0) {
                throw new UnprocessableError(`${failed.length} quality check(s) on ${stage.name} have not passed: ${failed
                    .map((f) => f.check_type.replace(/_/g, ' '))
                    .join(', ')}.`);
            }
            if (Number(stage.requires_quality_check) === 1 && checks.length === 0) {
                throw new UnprocessableError(`${stage.name} requires a quality check and none has been recorded. The published specification promises slump and cube testing on this work.`);
            }
            ...
            if (pendingCube) {
                throw new UnprocessableError(`The 28 day cube test for ${stage.name} is still pending. Certify after the result is in.`);
            }
        }
        await trx
...
export async function createSnag(db, actor, projectId, input) {
    return db.transaction().execute(async (trx) => {
        const row = await trx
            .insertInto('snags')
            .values({
            ...
            raised_on: today(),
            raised_source: input.raisedSource,
...
export async function setSnagStatus(db, actor, snagId, next) {
    await db.transaction().execute(async (trx) => {
        ...
        if (!allowed.includes(next)) {
            throw new UnprocessableError(`A ${snag.status.replace(/_/g, ' ')} snag cannot become ${next.replace(/_/g, ' ')}. Allowed: ${allowed.join(', ') || 'none'}.`);
        }
 ...
 */
export async function replaceTeam(db, actor, projectId, members) {
    await db.transaction().execute(async (trx) => {
        ...
        if (members.length > 0) {
            await trx
                ...
                project_id: projectId,
                user_id: m.userId,
                assignment_role: m.assignmentRole,
                from_date: today(),
                created_by: actor.userId,
            })))
                .execute();
...
export async function createApproval(db, actor, projectId, input) {
    return db.transaction().execute(async (trx) => {
        const row = await trx
 ...
 */
export async function addDocument(db, actor, projectId, input) {
    return db.transaction().execute(async (trx) => {
 ...
 */
export async function generateMilestones(trx, projectId, contractValuePaise, schedule) {
    if (schedule.length === 0)
        return;
    const sum = schedule.reduce((s, m) => s + m.percent, 0);
    if (Math.abs(sum - 100) > 0.01) {
        throw new UnprocessableError(`The payment schedule sums to ${sum.toFixed(2)} percent rather than 100. Correct it before creating milestones.`);
    }
    ...
    for (const m of schedule) {
        ...
        await trx
            ...
            amount_paise: contractValuePaise === null ? null : Math.round((contractValuePaise * m.percent) / 100),
