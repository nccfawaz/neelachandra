...
export async function listProjects(db, scope, opts) {
    ...
    if (opts.canViewCost)
        query = query.select('projects.contract_value_paise');
    if (opts.status)
        query = query.where('projects.status', '=', opts.status);
    if (scoped)
        query = query.where('projects.id', 'in', scoped.length ? scoped : [0]);
    return (await query.execute());
}
export async function findProject(db, id, canViewCost) {
    let query = db
        .selectFrom('projects')
        .innerJoin('clients', 'clients.id', 'projects.client_id')
        .select([
...
export async function projectStages(db, projectId) {
    return db
...
export async function projectMilestones(db, projectId, canViewCost) {
    let query = db
    ...
    if (canViewCost)
        query = query.select('project_milestones.amount_paise');
    return query.execute();
}
export async function projectDprs(db, projectId, limit = 30) {
    return db
 ...
 */
export async function stoppageSummary(db, projectId) {
    return db
        .selectFrom('daily_progress_reports')
        .select((eb) => [
        eb.ref('stoppage_reason').as('reason'),
        eb.fn.countAll().as('days'),
        eb.fn.sum('work_stopped_hours').as('hours'),
    ])
...
export async function projectSnags(db, projectId) {
    return db
        .selectFrom('snags')
...
export async function projectAssignments(db, projectId) {
    return db
...
export async function projectQualityChecks(db, projectId) {
    return db
...
export async function projectSafety(db, projectId) {
    return db
...
export async function projectDocuments(db, projectId) {
    return db
...
export async function projectBudgetVsActual(db, projectId) {
    ...
    const actualRows = await db
        ...
        eb.fn.sum('expense_lines.amount_paise').as('spent'),
    ])
...
export async function activeProjectOptions(db, scope) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db
        .selectFrom('projects')
        .select(['id', 'code', 'name'])
        ...
        .orderBy('code');
    if (scoped)
        query = query.where('id', 'in', scoped.length ? scoped : [0]);
    return query.execute();
}
/** Has a DPR already been filed for this project on this date. */
export async function dprExists(db, projectId, date) {
    const row = await db
        .selectFrom('daily_progress_reports')
        .select('id')
        .where('project_id', '=', projectId)
...
export async function openSnagCount(db, projectId) {
    const row = await db
        .selectFrom('snags')
...
export async function todayDprMissing(db, scope) {
    const scoped = await projectScopeFilter(db, scope);
    let query = db
        .selectFrom('projects')
...
export async function projectApprovals(db, projectId, canViewCost) {
    let query = db
    ...
    if (canViewCost)
        query = query.select('project_approvals.fee_paise');
    return query.execute();
}
/** Material issued to this project, for the materials tab. */
export async function projectMaterials(db, projectId, canViewCost) {
    let query = db
        ...
        eb.fn.sum('issue_lines.qty_issued').as('qty_issued'),
        eb.fn.sum('issue_lines.qty_returned').as('qty_returned'),
    ])
        ...
        .orderBy('items.name');
    if (canViewCost) {
        query = query.select((eb) => eb.fn
            .sum(eb(eb.ref('issue_lines.qty_issued'), '*', eb.ref('issue_lines.rate_paise')))
            .as('value_paise'));
    }
    return query.execute();
}
