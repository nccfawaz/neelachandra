...
export async function listLeads(db, scope, opts) {
    let query = db
        .selectFrom('leads')
    ...
    if (!scope.all) {
        query = query.where((eb) => eb.or([eb('leads.assigned_to', '=', scope.userId), eb('leads.assigned_to', 'is', null)]));
    }
    if (opts.stage)
        query = query.where('leads.stage', '=', opts.stage);
    if (opts.temperature)
        query = query.where('leads.temperature', '=', opts.temperature);
    if (opts.assignedTo)
        query = query.where('leads.assigned_to', '=', opts.assignedTo);
    if (opts.unassigned)
        query = query.where('leads.assigned_to', 'is', null);
    if (opts.source)
        query = query.where('leads.lead_source_id', '=', opts.source);
    if (opts.q) {
        const like = `%${opts.q}%`;
...
export async function countLeads(db, scope, opts) {
    let query = db.selectFrom('leads').select((eb) => eb.fn.countAll().as('n'));
    if (!scope.all) {
        query = query.where((eb) => eb.or([eb('leads.assigned_to', '=', scope.userId), eb('leads.assigned_to', 'is', null)]));
    }
    if (opts.stage)
 ...
 */
export async function findLead(db, id, canViewValue) {
    let query = db
        .selectFrom('leads')
 ...
 */
export async function leadVisible(db, scope, id) {
    if (scope.all) {
...
export async function leadActivities(db, leadId, limit = 50) {
    return db
...
export async function leadStageHistory(db, leadId) {
    return db
...
export async function leadVisits(db, leadId) {
    return db
 ...
 */
export async function hasCompletedVisit(db, leadId) {
    const row = await db
        .selectFrom('site_visits')
        .select('id')
...
export async function findVisit(db, id) {
    return db
        .selectFrom('site_visits')
...
export async function listVisits(db, scope, opts) {
    let query = db
        .selectFrom('site_visits')
        .innerJoin('leads', 'leads.id', 'site_visits.lead_id')
        .leftJoin('users', 'users.id', 'site_visits.visited_by')
    ...
    if (opts.status)
        query = query.where('site_visits.status', '=', opts.status);
    if (opts.from)
        query = query.where('site_visits.scheduled_at', '>=', `${opts.from} 00:00:00`);
    if (opts.to)
        query = query.where('site_visits.scheduled_at', '<=', `${opts.to} 23:59:59`);
    return query.execute();
}
export async function countVisits(db, scope, opts) {
    let query = db
        .selectFrom('site_visits')
        .innerJoin('leads', 'leads.id', 'site_visits.lead_id')
 ...
 */
export async function packageOptions(db, onDate = today()) {
    return db
...
export async function findEffectivePackage(db, id, onDate = today()) {
    return db
        .selectFrom('site_packages')
        .select(['id', 'name', 'slug', 'rate_per_sqft_paise', 'min_area_sqft', 'summary'])
        .where('id', '=', id)
 ...
 */
export async function packageSpec(db, packageId) {
    return db
...
export async function listQuotes(db, scope, opts) {
    let query = db
        .selectFrom('quotes')
    ...
    if (opts.status)
        query = query.where('quotes.status', '=', opts.status);
    if (opts.leadId)
        query = query.where('quotes.lead_id', '=', opts.leadId);
    if (opts.q) {
        const like = `%${opts.q}%`;
        query = query.where((eb) => eb.or([eb('quotes.quote_no', 'like', like), eb('leads.contact_name', 'like', like)]));
    }
    return query.execute();
}
export async function countQuotes(db, scope, opts) {
    let query = db
        .selectFrom('quotes')
        .innerJoin('leads', 'leads.id', 'quotes.lead_id')
        .select((eb) => eb.fn.countAll().as('n'));
...
export async function findQuote(db, id) {
    return db
        .selectFrom('quotes')
        .innerJoin('leads', 'leads.id', 'quotes.lead_id')
...
export async function quoteLines(db, quoteId) {
    return db
...
export async function quoteRevisions(db, quoteNo) {
    return db
        .selectFrom('quotes')
...
export async function leadQuotes(db, leadId) {
    return db
        .selectFrom('quotes')
        .select([
        'id',
 ...
 */
export async function acceptedQuotes(db, leadId) {
    return db
        .selectFrom('quotes')
 ...
 * than a table scan.
 */
const notDormant = (onDate) => sql `COALESCE((SELECT MAX(la.occurred_at) FROM lead_activities la WHERE la.lead_id = leads.id), leads.created_at) >= DATE_SUB(${onDate}, INTERVAL ${sql.lit(DORMANT_DAYS)} DAY)`;
/**
 ...
 */
export async function pipelineTotals(db, scope, onDate = today()) {
    let query = db
        .selectFrom('leads')
        .select((eb) => [
        'leads.stage',
        eb.fn.countAll().as('n'),
        sql `COALESCE(SUM(leads.expected_value_paise), 0)`.as('value_paise'),
        sql `COALESCE(SUM(leads.expected_value_paise * COALESCE(leads.probability_pct, 0) / 100), 0)`.as('weighted_paise'),
    ])
 ...
 */
export async function boardCards(db, scope, opts) {
    let query = db
        .selectFrom('leads')
        .leftJoin('users', 'users.id', 'leads.assigned_to')
        .select([
...
export async function crmKpis(db, scope, opts) {
    ...
    let followupsQ = db
        ...
        .where('stage', 'in', OPEN_STAGES);
    if (!scope.all) {
        followupsQ = followupsQ.where((eb) => eb.or([eb('assigned_to', '=', scope.userId), eb('assigned_to', 'is', null)]));
    }
    let visitsQ = db
        .selectFrom('site_visits')
        .innerJoin('leads', 'leads.id', 'site_visits.lead_id')
    ...
    if (!scope.all) {
        visitsQ = visitsQ.where((eb) => eb.or([eb('leads.assigned_to', '=', scope.userId), eb('leads.assigned_to', 'is', null)]));
    }
...
export async function dueFollowups(db, scope, onDate = today()) {
    let query = db
        .selectFrom('leads')
        .leftJoin('users', 'users.id', 'leads.assigned_to')
        .select([
 ...
 */
export async function firstResponseBreaches(db, targetHours, scope = null) {
    let query = db
        .selectFrom('leads')
        .leftJoin('users', 'users.id', 'leads.assigned_to')
        .select([
        ...
        .where('leads.stage', 'in', OPEN_STAGES)
        .where(sql `leads.created_at < DATE_SUB(NOW(), INTERVAL ${sql.lit(targetHours)} HOUR)`)
...
export async function dormantCandidates(db, onDate = today()) {
    return db
        .selectFrom('leads')
        .select(['id', 'lead_no', 'stage'])
        .where('stage', 'in', OPEN_STAGES)
        .where(sql `NOT (${notDormant(onDate)})`)
        .orderBy('id')
        .limit(500)
        .execute();
}
 ...
 */
export async function funnelReport(db, from, to) {
    const rows = await db
        .selectFrom('lead_stage_history')
        .select([
        'lead_stage_history.to_stage',
        sql `COUNT(DISTINCT lead_stage_history.lead_id)`.as('leads'),
        sql `ROUND(AVG(lead_stage_history.days_in_previous_stage), 1)`.as('avg_days_in_previous'),
    ])
 ...
 */
export async function sourceReport(db, from, to) {
    const rows = await db
        .selectFrom('leads')
        .leftJoin('lead_sources', 'lead_sources.id', 'leads.lead_source_id')
        .leftJoin('projects', 'projects.id', 'leads.converted_project_id')
        ...
        'lead_sources.name as source_name',
        sql `COUNT(*)`.as('leads'),
        sql `SUM(CASE WHEN leads.stage = 'won' THEN 1 ELSE 0 END)`.as('won'),
        sql `SUM(CASE WHEN leads.stage = 'lost' THEN 1 ELSE 0 END)`.as('lost'),
        sql `COALESCE(SUM(CASE WHEN leads.stage = 'won' THEN projects.contract_value_paise ELSE 0 END), 0)`.as('won_value_paise'),
    ])
 ...
 */
export async function lossReport(db, from, to) {
    const rows = await db
        .selectFrom('leads')
        .select([
        'leads.lost_reason',
...
export async function listCompetitors(db) {
    return db
 ...
 */
export async function enquiriesWithoutLead(db, limit = 100) {
    return db
...
export async function findEnquiry(db, id) {
    return db
        .selectFrom('enquiries')
        .select([
        'id',
 ...
 */
export async function leadSourceOptions(db) {
    return db
...
export async function campaignOptions(db) {
    return db
...
export async function clientOptions(db) {
    return db
        .selectFrom('clients')
        .select(['id', 'code', 'name'])
        .where('status', '=', 'active')
 ...
 */
export async function assignableUsers(db) {
    return db
        .selectFrom('users')
...
export async function stageTemplateOptions(db) {
    return db
