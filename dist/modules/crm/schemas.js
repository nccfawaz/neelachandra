 ...
 */
const optionalEnum = (values) => z
    .string()
    .optional()
    .transform((v) => (v === '' || v === undefined ? null : v))
    .refine((v) => v === null || values.includes(v), 'That choice is not one of the options.')
    .transform((v) => v);
/**
...
export const JURISDICTIONS = ['BBMP', 'BMRDA', 'BDA', 'Gram Panchayat', 'TUDA', 'KIADB', 'Other'];
...
export const FUNDING_MODES = ['self', 'home_loan', 'loan_sanctioned', 'loan_applied', 'company_capex'];
...
export const VISIT_STATUSES = ['scheduled', 'completed', 'client_no_show', 'rescheduled', 'cancelled'];
export const ROAD_ACCESS = ['good', 'narrow', 'no_access'];
export const WATER_AVAILABILITY = ['borewell', 'corporation', 'tanker', 'none'];
export const FEASIBILITIES = ['feasible', 'feasible_with_conditions', 'not_feasible'];
...
export const PRICING_BASES = ['per_sqft', 'item_rate', 'lumpsum'];
...
export const leadSchema = z.object({
    ...
    .refine((v) => v.budgetMinPaise === null || v.budgetMaxPaise === null || v.budgetMaxPaise >= v.budgetMinPaise, 'The budget ceiling is below the floor.');
/**
...
export const QUOTE_LINE_TYPES = ['addon', 'extra_work'];
/**
...
export const quoteSchema = z
    ...
    for (let i = 0; i < v.lineDescription.length; i += 1) {
        ...
        if (description === null)
            continue;
        const kind = (v.lineType[i] ?? 'addon');
        if (!QUOTE_LINE_TYPES.includes(kind)) {
 ...
 * the conversion form.
 */
export const DELIVERY_MODEL_FOR_BASIS = {
