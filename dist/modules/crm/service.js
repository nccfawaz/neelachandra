...
import { ensureSiteStore, generateMilestones, instantiateStagesFromTemplate, resolveStageTemplate, } from '../projects/service.js';
...
export function computeLeadScore(lead) {
    ...
    signals.push({
        ...
        points: city !== '' && SERVED_CITIES.includes(city) ? 10 : 0,
        max: 10,
    });
 ...
 */
export function temperatureFor(score, daysSinceLastActivity) {
...
export function advances(from, to) {
...
export function expectedValuePaise(input) {
    if (input.packageRatePaise !== null &&
 ...
 * prevent.
 */
async function rescoreLead(trx, leadId) {
    ...
    const packageRatePaise = lead.package_rate_paise === null ? null : Number(lead.package_rate_paise);
    const targetBuiltUpSqft = lead.target_built_up_sqft === null ? null : Number(lead.target_built_up_sqft);
    ...
    const lastAt = last?.last_at ?? lead.created_at;
 ...
 */
export async function duplicatesByPhone(db, phone, exceptLeadId = null) {
    let q = db
        .selectFrom('leads')
    ...
    if (exceptLeadId !== null)
        q = q.where('id', '!=', exceptLeadId);
    return q.execute();
}
export async function createLead(db, actor, input, assignedTo = null) {
...
export async function updateLead(db, actor, leadId, input) {
    return db.transaction().execute(async (trx) => {
        ...
        if (!before)
            throw new NotFoundError('That lead does not exist.');
        if (before.stage === 'won') {
            throw new ConflictError('This lead has been converted to a project. Edit the project rather than the lead it came from.');
        }
        await trx
            .updateTable('leads')
 ...
 */
export async function leadFromEnquiry(db, actor, enquiryId, assignedTo) {
    return db.transaction().execute(async (trx) => {
        const enq = await trx
            ...
            .executeTakeFirst();
        if (!enq)
            throw new NotFoundError('That enquiry does not exist.');
        if (enq.status === 'spam') {
 ...
 */
export async function changeStage(db, actor, leadId, opts) {
    if (opts.stage === 'won') {
        throw new UnprocessableError('A lead becomes won by converting it to a project, which is what creates the client and the contract.');
    }
    if (opts.stage === 'lost') {
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (TERMINAL_STAGES.includes(lead.stage)) {
            throw new ConflictError(`This lead is ${lead.stage}, which is final. ${lead.stage === 'won' ? 'Work on the project it became.' : 'Create a new lead if the client comes back.'}`);
        }
        ...
        if (opts.stage === 'quote_sent' && !(await hasCompletedVisit(trx, leadId))) {
            throw new UnprocessableError('No completed site visit with a feasibility verdict is on record. A per-square-foot rate quoted without seeing the plot is how a level difference or an approach road no transit mixer fits down turns a profitable job into a loss.');
        }
 ...
 */
export async function assignLead(db, actor, leadId, opts) {
    await db.transaction().execute(async (trx) => {
        ...
        if (before === opts.assignedTo) {
            throw new ConflictError(opts.assignedTo === null ? 'This lead is already in the pool.' : 'This lead is already assigned to that person.');
        }
 ...
 */
export async function logActivity(db, actor, leadId, input) {
    return db.transaction().execute(async (trx) => {
        const lead = await trx
            .selectFrom('leads')
        ...
        const firstResponse = lead.first_response_at === null &&
            OUTBOUND_ACTIVITY_TYPES.includes(input.activityType);
        const set = { updated_by: actor.userId };
        if (firstResponse)
            set.first_response_at = input.occurredAt;
        if (input.nextAction !== null)
            set.next_action = input.nextAction;
        if (input.nextActionDate !== null)
            set.next_action_date = input.nextActionDate;
 ...
 */
export async function scheduleVisit(db, actor, opts) {
    return db.transaction().execute(async (trx) => {
        const lead = await trx
            .selectFrom('leads')
        ...
        if (!visitId)
            throw new Error('Site visit insert returned no id');
 ...
 */
export async function completeVisit(db, actor, visitId, input) {
    return db.transaction().execute(async (trx) => {
        const visit = await trx
            ...
            .executeTakeFirst();
        if (!visit)
            throw new NotFoundError('That site visit does not exist.');
        ...
        await trx
            ...
            summary: `Site visit: ${input.feasibility.replace(/_/g, ' ')}${input.estimatedExtraCostPaise ? `, extras ${formatPaiseAsRupees(input.estimatedExtraCostPaise)}` : ''}${input.conditionsNotes ? ` — ${input.conditionsNotes}` : ''}`.slice(0, 500),
            created_by: actor.userId,
        })
            .execute();
        await rescoreLead(trx, leadId);
 ...
 */
export async function setVisitStatus(db, actor, visitId, opts) {
    if (opts.status === 'completed') {
        throw new UnprocessableError('Complete a visit through the feasibility form, so the findings that a quote depends on are on the record.');
    }
    await db.transaction().execute(async (trx) => {
        const visit = await trx
            .selectFrom('site_visits')
 ...
 */
export async function createQuote(db, actor, input) {
    return db.transaction().execute(async (trx) => {
 ...
 */
async function insertPricedQuote(trx, actor, input, meta) {
    {
        const lead = await trx
            .selectFrom('leads')
        ...
        if (!(await hasCompletedVisit(trx, input.leadId))) {
            throw new UnprocessableError('This lead has no completed site visit with a feasibility verdict. Quoting a per-square-foot rate without seeing the plot is how a 4 ft level difference or a 12 ft approach road turns a profitable job into a loss.');
        }
        ...
        if (input.packageId !== null) {
            ...
            if (!pkg) {
                throw new UnprocessableError('That package has no rate in effect on the quote date. Pick another package or set the rate by hand.');
            }
            packageName = pkg.name;
            if (ratePerSqftPaise === null)
                ratePerSqftPaise = Number(pkg.rate_per_sqft_paise);
            if (input.builtUpAreaSqft !== null &&
                pkg.min_area_sqft !== null &&
                input.builtUpAreaSqft < Number(pkg.min_area_sqft)) {
                throw new UnprocessableError(`${pkg.name} applies from ${Number(pkg.min_area_sqft)} sq ft. This quote is for ${input.builtUpAreaSqft} sq ft.`);
            }
        }
        if (input.pricingBasis === 'per_sqft' && (ratePerSqftPaise === null || ratePerSqftPaise <= 0)) {
            throw new UnprocessableError('A per-square-foot quote needs a rate. Choose a package or enter the rate.');
        }
        ...
        const visit = await trx
            ...
            .orderBy('id', 'desc')
            .executeTakeFirst();
        const visitExtrasPaise = visit?.estimated_extra_cost_paise === null || visit?.estimated_extra_cost_paise === undefined
            ? 0
        ...
        if (!quoteId)
            throw new Error('Quote insert returned no id');
 ...
 */
async function writeQuoteLines(trx, quoteId, opts) {
    let sort = 0;
 ...
 */
export async function submitQuote(db, actor, quoteId, roleKeys) {
    return db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'draft') {
            throw new ConflictError(`This quote is ${quote.status.replace(/_/g, ' ')}. Only a draft is submitted for approval.`);
        }
...
async function loadQuoteForUpdate(trx, quoteId) {
 ...
 */
export async function approveQuote(db, actor, quoteId) {
    return db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'pending_approval') {
            throw new ConflictError(`This quote is ${quote.status.replace(/_/g, ' ')}. Only one awaiting approval can be approved.`);
        }
        if (Number(quote.created_by) === actor.userId) {
            throw new UnprocessableError('You raised this quote, so you cannot approve its discount. That is what the escalation is for.');
        }
        await trx
            .updateTable('quotes')
            .set({
 ...
 */
export async function declineQuote(db, actor, quoteId, reason) {
    return db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'pending_approval') {
            throw new ConflictError(`This quote is ${quote.status.replace(/_/g, ' ')}. Only one awaiting approval can be declined.`);
        }
        await trx
            .updateTable('quotes')
 ...
 */
export async function sendQuote(db, actor, quoteId) {
    const prepared = await db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'approved') {
            throw new ConflictError(quote.status === 'sent' || quote.status === 'viewed'
        ...
        if (quote.valid_until < today()) {
            throw new UnprocessableError(`This quote expired on ${quote.valid_until}. Revise it with a new validity date rather than sending a dead price.`);
        }
        ...
        if (!(await hasCompletedVisit(trx, Number(quote.lead_id)))) {
            throw new UnprocessableError('This lead has no completed site visit with a feasibility verdict, so the rate behind this quote has not been checked against the plot.');
        }
        const lead = await trx
            .selectFrom('leads')
            ...
            .executeTakeFirst();
        if (!lead)
            throw new NotFoundError('That quote points at a lead that no longer exists.');
        if (TERMINAL_STAGES.includes(lead.stage)) {
            throw new ConflictError(`This lead is ${lead.stage}. A closed lead is not quoted — a client who comes back is a new lead.`);
        }
        const now = nowSqlDateTime();
        await trx
            .updateTable('quotes')
 ...
 */
export async function markQuoteViewed(db, quoteId) {
 ...
 */
export async function acceptQuote(db, actor, quoteId, note) {
    return db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'sent' && quote.status !== 'viewed') {
            throw new ConflictError(quote.status === 'accepted'
 ...
 */
export async function rejectQuote(db, actor, quoteId, reason) {
    return db.transaction().execute(async (trx) => {
        const quote = await loadQuoteForUpdate(trx, quoteId);
        if (quote.status !== 'sent' && quote.status !== 'viewed') {
            throw new ConflictError(`This quote is ${quote.status.replace(/_/g, ' ')}. Only one that has gone to the client can be rejected.`);
        }
        const leadId = Number(quote.lead_id);
        await trx
            .updateTable('quotes')
 ...
 */
export async function reviseQuote(db, actor, quoteId, input) {
    return db.transaction().execute(async (trx) => {
        const old = await loadQuoteForUpdate(trx, quoteId);
        if (old.status === 'superseded') {
            throw new ConflictError('This quote has already been superseded. Revise the current revision instead.');
        }
        if (old.status === 'accepted') {
            throw new ConflictError('This quote has been accepted, so it is the agreed price. A change now is a new quote, not a revision of the one the client said yes to.');
        }
 ...
 * calls back.
 */
export async function loseLead(db, actor, leadId, input) {
    return db.transaction().execute(async (trx) => {
        const lead = await trx
            .selectFrom('leads')
        ...
        if (!lead)
            throw new NotFoundError('That lead does not exist.');
        if (lead.stage === 'won') {
            throw new ConflictError('This lead was won and converted to a project. Closing the project is a different act, in the projects module.');
        }
        if (lead.stage === 'lost') {
            throw new ConflictError('This lead is already recorded as lost.');
        }
        const competitorId = await upsertCompetitor(trx, input.lostToCompetitor, input.competitorRatePerSqftPaise);
        const now = nowSqlDateTime();
        await trx
            .updateTable('leads')
            .set({
 ...
 */
async function upsertCompetitor(trx, name, ratePerSqftPaise) {
    if (name === null)
        return null;
    ...
    const existing = await trx
        ...
        .where(sql `LOWER(name) = LOWER(${trimmed})`)
        .executeTakeFirst();
    if (existing) {
        if (ratePerSqftPaise !== null) {
            await trx
 ...
 */
export async function setProbability(db, actor, leadId, input) {
    return db.transaction().execute(async (trx) => {
        const lead = await trx
            .selectFrom('leads')
 ...
 * forever.
 */
const PROJECT_TYPE_FOR_ENQUIRY = {
 ...
 */
export async function convertLeadToProject(db, actor, leadId, overrides = {
    plannedStart: null,
    ...
    return db.transaction().execute(async (trx) => {
        ...
        if (lead.converted_project_id !== null) {
            throw new ConflictError(`This lead has already been converted. Its project is ${await projectCodeOf(trx, Number(lead.converted_project_id))}.`);
        }
        if (lead.stage === 'lost') {
        ...
        if (projectType === null) {
            throw new UnprocessableError('A consultation is not a project. If this became a build, raise a lead for the build with the work it covers.');
        }
        const quote = await trx
            .selectFrom('quotes')
            .select([
            ...
            .executeTakeFirst();
        if (!quote) {
            throw new UnprocessableError('This lead has no accepted quote. A project is opened against the price the client agreed to, so the quote has to be marked accepted first.');
        }
        if (lead.site_city === null || lead.site_city.trim().length === 0) {
            throw new UnprocessableError('The lead has no site city. A project needs one: it is on the site store, on every delivery challan and on the GST invoice.');
        }
        ...
        const inserted = await trx
            ...
            delivery_model: DELIVERY_MODEL_FOR_BASIS[quote.pricing_basis],
        ...
        if (schedule.length === 0) {
            throw new UnprocessableError(`Quote ${quote.quote_no} has no payment schedule, so there is nothing to raise invoices against. Revise it with a schedule before converting.`);
        }
        await generateMilestones(trx, projectId, contractValuePaise, schedule);
        await ensureSiteStore(trx, projectId, projectCode, `${lead.contact_name} — ${lead.site_locality ?? lead.site_city}`, lead.site_city, siteAddressOf(lead.site_locality, lead.site_city));
        const now = nowSqlDateTime();
        await trx
            .updateTable('leads')
            .set({
...
async function projectCodeOf(trx, projectId) {
    const row = await trx
        .selectFrom('projects')
        .select('code')
        .where('id', '=', projectId)
 ...
 */
function siteAddressOf(locality, city) {
    const parts = [locality, city].filter((p) => p !== null && p.trim().length > 0);
    return parts.join(', ');
}
/**
 ...
 */
function parsePaymentSchedule(raw) {
    ...
    for (const item of parsed) {
        ...
        if (name.length === 0 || !Number.isFinite(percent) || percent <= 0)
            continue;
 ...
 */
async function upsertClientFromLead(trx, actor, lead) {
    if (lead.clientId !== null) {
        const existing = await trx
            ...
            .executeTakeFirst();
        if (existing)
            return { clientId: Number(existing.id), created: false };
    }
    ...
    if (byPhone) {
        if (byPhone.status === 'blacklisted') {
            throw new ConflictError('This phone number belongs to a blacklisted client. Clear the client record before opening a project for them.');
        }
        return { clientId: Number(byPhone.id), created: false };
    }
    const isCompany = lead.enquiryType === 'commercial_construction' || lead.enquiryType === 'industrial_construction';
    ...
    if (!clientId)
        throw new Error('Client insert returned no id');
    await trx
        .updateTable('clients')
 ...
 */
export async function runCrmFollowups(db) {
    const on = today();
    ...
    const OPEN_STAGES = ['new', 'contacted', 'qualified', 'site_visit_scheduled', 'site_visit_done', 'estimate_shared', 'quote_sent', 'negotiation', 'verbal_agreement'];
    ...
    const stale = await db
        ...
        .having(sql `COALESCE(MAX(lead_activities.occurred_at), leads.created_at) < ${cutoff}`)
        .execute();
    if (stale.length > 0) {
        await db
            .updateTable('leads')
            .set({ stage: 'dormant', stage_changed_at: nowSqlDateTime(), temperature: 'cold', probability_pct: 0 })
            .where('id', 'in', stale.map((l) => Number(l.id)))
            .execute();
    ...
    if (enquiryCount > 0 || leadPoolCount > 0) {
        ...
        const parts = [];
        if (enquiryCount > 0)
            parts.push(`${enquiryCount} website ${enquiryCount === 1 ? 'enquiry has' : 'enquiries have'} not been promoted to a lead`);
        if (leadPoolCount > 0)
            parts.push(`${leadPoolCount} ${leadPoolCount === 1 ? 'lead is' : 'leads are'} in the unassigned pool`);
        await notify(db, {
            userIds: assigners,
            kind: 'crm_unassigned',
 ...
 */
async function notifyPerAssignee(db, overdue, nearExpiry) {
    const byUser = new Map();
    const bucket = (userId) => {
        const found = byUser.get(userId);
        if (found)
            return found;
        const fresh = { overdue: [], quotes: [] };
    ...
    for (const lead of overdue) {
        if (lead.assigned_to === null)
            continue;
        bucket(Number(lead.assigned_to)).overdue.push(`${lead.lead_no} ${lead.contact_name} — ${lead.next_action ?? 'follow up'} (due ${lead.next_action_date ?? 'unknown'})`);
    }
    for (const quote of nearExpiry) {
        ...
        bucket(owner).quotes.push(`${quote.quote_no} rev ${quote.revision} for ${quote.contact_name}, ${formatPaiseAsRupees(Number(quote.total_paise))}, expires ${quote.valid_until}`);
    }
    let sent = 0;
    for (const [userId, work] of byUser) {
        ...
        if (work.overdue.length > 0) {
            ...
            if (work.overdue.length > 12)
                lines.push(`…and ${work.overdue.length - 12} more.`);
        }
        if (work.quotes.length > 0) {
            ...
            if (work.quotes.length > 12)
                lines.push(`…and ${work.quotes.length - 12} more.`);
        }
        if (lines.length === 0)
            continue;
        await notify(db, {
            userIds: [userId],
            kind: 'crm_followups',
            title: work.overdue.length > 0 && work.quotes.length > 0
