 ...
 */
async function nextRevisionNo(db, pageId) {
    const row = await db
        .selectFrom('site_page_revisions')
        .select((eb) => eb.fn.max('revision_no').as('max_no'))
        .where('page_id', '=', pageId)
        .executeTakeFirst();
    return Number(row?.max_no ?? 0) + 1;
}
function assertJsonSafe(value, column) {
 ...
 */
export async function writeRevision(db, actor, pageId, changeNote, preferDraft) {
    const page = await db
    ...
    const schemaRaw = preferDraft && row.draft_schema_types != null ? row.draft_schema_types : row.schema_types;
    const contentRaw = preferDraft && row.draft_content_json != null ? row.draft_content_json : row.content_json;
 ...
 */
export async function editPage(db, actor, pageId, input) {
    return await db.transaction().execute(async (trx) => {
        ...
        const hasDraft = await trx
            ...
            .where('id', '=', pageId)
            .executeTakeFirst();
        const revisionId = await writeRevision(trx, actor, pageId, input.changeNote ?? null, hasDraft?.draft_content_json != null);
 ...
 */
export async function publishPage(db, actor, pageId) {
    return await db.transaction().execute(async (trx) => {
        await trx
            .selectFrom('site_pages')
            .select('id')
 ...
 */
export async function revertToRevision(db, actor, pageId, revisionNo) {
    return await db.transaction().execute(async (trx) => {
        await trx
            .selectFrom('site_pages')
            .select('id')
        ...
        const hasDraft = await trx
            ...
            .where('id', '=', pageId)
            .executeTakeFirst();
        await writeRevision(trx, actor, pageId, `before revert to revision ${revisionNo}`, hasDraft?.draft_content_json != null);
        const revision = await trx
            ...
            .executeTakeFirst();
        if (!revision)
            throw new NotFoundError(`Revision ${revisionNo} does not exist for this page`);
        ...
        const schemaTypes = parseJsonColumn(revision.schema_types);
        const contentJson = parseJsonColumn(revision.content_json);
        if (schemaTypes === null || contentJson === null) {
            throw new UnprocessableError(`Revision ${revisionNo} holds a NULL snapshot cell and cannot be restored — this shape is refused by migration 026 (DECISIONS 21.4)`);
        }
        await trx
            .updateTable('site_pages')
            .set({
