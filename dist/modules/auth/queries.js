...
export async function recordAttempt(db, opts) {
    await db
 ...
 */
export async function failureCounts(db, email, ip) {
...
export async function setLock(db, userId, until) {
    await db.updateTable('users').set({ locked_until: until }).where('id', '=', userId).execute();
}
export async function bumpFailure(db, userId) {
    await db
        .updateTable('users')
...
export async function markLoginSuccess(db, userId, ip) {
    await db
        .updateTable('users')
        .set({
...
export async function insertToken(db, opts) {
    await db
...
export async function findLiveToken(db, tokenHash) {
    const row = await db
...
export async function consumeToken(trx, tokenId) {
    await trx
...
export async function invalidateOtherTokens(trx, userId, exceptId) {
    await trx
        .updateTable('password_reset_tokens')
        .set({ used_at: nowSqlDateTime() })
...
export async function setPassword(trx, userId, hash) {
    await trx
        .updateTable('users')
        .set({
        password_hash: hash,
...
export async function setTotpSecret(trx, userId, secret, confirmed) {
    await trx
        .updateTable('users')
        .set({
        totp_secret: secret,
...
export async function ownerEmails(db) {
    const rows = await db
        .selectFrom('users')
