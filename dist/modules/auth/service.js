 ...
 */
async function applyLockout(db, user, ip) {
...
async function maybeReissueInvite(db, user) {
    const live = await db
...
/* Invites and resets ----------------------------------------------------- */
export async function issueInvite(db, userId, email, fullName, ip) {
...
export async function requestReset(opts) {
    ...
    const link = `${env.APP_BASE_URL}/reset-password/${token}`;
    const mail = user.password_hash === null
...
export async function completeReset(opts) {
    ...
    if (!user)
        throw new UnprocessableError('That link is no longer valid.');
...
export async function changeOwnPassword(opts) {
    ...
    if (!user)
        throw new UnprocessableError('Account not found.');
    if (user.password_hash !== null) {
        if (!opts.current)
            throw new UnprocessableError('Enter your current password.');
        const ok = await verifyPassword(user.password_hash, opts.current);
        if (!ok)
            throw new UnprocessableError('Your current password is not correct.');
    }
    await assertPasswordPolicy(opts.password, { email: user.email, fullName: user.full_name });
    const hash = await hashPassword(opts.password);
...
export async function verifyTotp(opts) {
    ...
    if (!limited.allowed) {
        throw new UnprocessableError(`Too many codes tried. Wait ${Math.ceil(limited.retryAfterSeconds / 60)} minutes.`);
    }
    const user = await q.findUserById(db, opts.userId);
    ...
    return db.transaction().execute(async (trx) => {
        ...
        await clearBucket(trx, RULES.totpByUser(opts.userId));
        const rotated = await rotateSession(trx, opts.sessionId, {
            totpVerified: true,
            ip: opts.ip,
            userAgent: opts.userAgent,
