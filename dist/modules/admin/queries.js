...
export async function allRoles(db) {
    return db
        .selectFrom('roles')
        .select([
        'id',
...
export async function permissionIdsForRole(db, roleId) {
    const rows = await db
...
export async function overridesFor(db, userId) {
    return db
 ...
 */
export async function otherActiveOwnerCount(db, excludeUserId) {
    const row = await db
...
export async function approvalLimits(db) {
...
export async function auditPage(db, filter) {
    let q = db
    ...
    if (filter.userId !== undefined)
        q = q.where('audit_log.user_id', '=', filter.userId);
    if (filter.action)
        q = q.where('audit_log.action', '=', filter.action);
    if (filter.entityType)
        q = q.where('audit_log.entity_type', '=', filter.entityType);
    if (filter.from)
        q = q.where('audit_log.created_at', '>=', `${filter.from} 00:00:00`);
    if (filter.to)
        q = q.where('audit_log.created_at', '<=', `${filter.to} 23:59:59`);
    return q.limit(filter.limit).offset(filter.offset).execute();
}
export async function auditCount(db, filter) {
    let q = db.selectFrom('audit_log').select((eb) => eb.fn.countAll().as('n'));
    if (filter.userId !== undefined)
...
export async function enquiryPage(db, filter) {
    ...
    if (filter.status)
        q = q.where('enquiries.status', '=', filter.status);
    return q.limit(filter.limit).offset(filter.offset).execute();
}
export async function enquiryCount(db, status) {
    let q = db.selectFrom('enquiries').select((eb) => eb.fn.countAll().as('n'));
    if (status)
        q = q.where('status', '=', status);
    const row = await q.executeTakeFirst();
    return Number(row?.n ?? 0);
}
export async function referenceCounts(db) {
    const [costHeads, units, numbering] = await Promise.all([
        db.selectFrom('cost_heads').select((eb) => eb.fn.countAll().as('n')).executeTakeFirst(),
        db.selectFrom('units').select((eb) => eb.fn.countAll().as('n')).executeTakeFirst(),
        db.selectFrom('document_numbering').select((eb) => eb.fn.countAll().as('n')).executeTakeFirst(),
    ]);
    return {
...
export async function listCostHeads(db) {
    return db
...
export async function listNumbering(db) {
    return db
