...
const ZIP_DECLARED_TYPES = {
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': 'xlsx',
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document': 'docx',
    'application/vnd.openxmlformats-officedocument.presentationml.presentation': 'pptx',
};
export function sniff(head, declaredType) {
    ...
    throw new UnprocessableError('That file type is not accepted. Upload a JPEG, PNG, WebP, HEIC, PDF or Office document.');
}
/**
 ...
 */
export async function storeUpload(db, file, opts) {
    if (file.size <= 0)
        throw new BadRequestError('That file is empty.');
    if (file.size > MAX_UPLOAD_BYTES) {
        throw new UnprocessableError(`That file is ${(file.size / 1024 / 1024).toFixed(1)} MB. The limit is 15 MB. Photos taken on a phone can be resized before upload.`);
    }
...
export function absolutePathFor(row) {
...
export async function deleteStoredFile(db, fileId) {
    const row = await db
        .selectFrom('files')
