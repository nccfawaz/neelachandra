...
const RECOVERY_ARGON = { memoryCost: 19456, timeCost: 2, parallelism: 1 };
export async function storeRecoveryCodes(trx, userId, codes) {
 ...
 */
export async function consumeRecoveryCode(trx, userId, supplied) {
...
export async function countUnusedRecoveryCodes(db, userId) {
    const row = await db
        .selectFrom('user_recovery_codes')
