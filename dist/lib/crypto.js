 ...
 */
export function encryptToBuffer(plaintext) {
    const iv = randomBytes(12);
    const cipher = createCipheriv('aes-256-gcm', key(), iv);
    const ct = Buffer.concat([cipher.update(plaintext, 'utf8'), cipher.final()]);
    return Buffer.concat([iv, cipher.getAuthTag(), ct]);
}
export function decryptFromBuffer(blob) {
    if (blob.length < 29) {
...
export function bufferToIp(buf) {
    if (!buf)
        return null;
    if (buf.length === 4)
...
function expandIpv6(addr) {
    ...
    if (addr.includes('::')) {
        ...
        const all = [...headGroups, ...Array(fill).fill('0'), ...tailGroups];
