 ...
 */
let transporter;
let transportUnavailableReason;
function getTransport() {
    if (transporter)
        return transporter;
    if (transportUnavailableReason)
        return undefined;
...
export async function send(db, opts) {
    ...
    if (!transport) {
        ...
        if (!isProd) {
            ...
            console.warn(`[mailer] Not sent (${reason}). To: ${opts.to} Subject: ${opts.subject}\n${opts.text}`);
        }
...
async function logEmail(db, opts, status, response, errorMessage) {
    try {
        await db
...
export function lockoutAlertEmail(opts) {
    const text = [
