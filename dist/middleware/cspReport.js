...
function parseViolation(body) {
    ...
    if (!out['violated-directive'] && !out['effective-directive'])
        return null;
    if (!out['document-uri'])
        return null;
    return out;
...
export function resetCspRateLimit() {
    rateBuckets.clear();
}
    ...
    if (!bucket || now - bucket.windowStart >= CSP_RATE_WINDOW_MS) {
        ...
        if (rateBuckets.size > 1000) {
            for (const [k, b] of rateBuckets) {
                if (now - b.windowStart >= 2 * CSP_RATE_WINDOW_MS)
...
export function cspReportOnly() {
    return async function cspReportOnly(_c, next) {
        await next();
        const c = _c;
