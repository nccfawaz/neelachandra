...
const SUBSTITUTE = { 409: 400, 422: 400 };
const SITE_ROOT = process.env.NCC_SITE_ROOT ?? process.cwd();
const cache = new Map();
async function staticErrorPage(code) {
 ...
 */
function fallbackPage(code, copy) {
    return `<!DOCTYPE html>
...
async function respond(c, code, message, detail) {
    ...
    if (c.req.header('hx-request')) {
        return c.html(`<div class="ncc-alert ncc-alert--error" role="alert"><strong>${copy.title}.</strong> ${message}</div>`);
    }
...
export const errorHandler = async (err, c) => {
    ...
    if (code >= 500) {
        console.error(`[${c.get('requestId') ?? '-'}] ${c.req.method} ${new URL(c.req.url).pathname} -> ${code}`, err);
    }
